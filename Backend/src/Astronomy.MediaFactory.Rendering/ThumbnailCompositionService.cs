...
public sealed class ThumbnailCompositionService : IThumbnailCompositionService
{
    ...
    public async Task<string> ComposeAsync(ThumbnailCompositionRequest request, CancellationToken cancellationToken)
    {
        ...
        var report = new CinematicThumbnailAiReport
        {
            ...
            AtmosphereDepthScore = Math.Round(Math.Clamp(request.SelectedCandidate.EnvironmentalDepthScore * 0.55d + request.SelectedCandidate.AtmosphereContinuityScore * 0.45d, 0, 1), 3),
            FogBlendScore = Math.Round(Math.Clamp(request.SelectedCandidate.OrganicAtmosphereScore * 0.62d + hierarchy.ReadabilityScore * 0.38d, 0, 1), 3),
            ProceduralArtifactPenalty = Math.Round(Math.Clamp(request.SelectedCandidate.CompositingVisibilityPenalty * 0.55d + request.SelectedCandidate.VisualArtifactPenalty * 0.45d, 0, 1), 3),
            CinematicSoftnessScore = Math.Round(Math.Clamp(request.SelectedCandidate.CinematicSubtletyScore * 0.58d + request.SelectedCandidate.EdgeIntegrationScore * 0.42d, 0, 1), 3),
            AtmosphericRealismScore = Math.Round(Math.Clamp(request.SelectedCandidate.EnvironmentalDepthScore * 0.28d + request.SelectedCandidate.OrganicAtmosphereScore * 0.24d + request.SelectedCandidate.EdgeIntegrationScore * 0.22d + request.SelectedCandidate.CinematicSubtletyScore * 0.18d + hierarchy.ReadabilityScore * 0.08d, 0, 1), 3),
