    ...
    : IDocumentaryBlueprintCertificationIntegrationService
{
    ...
    public Task<DocumentaryBlueprintCertificationIntegrationResult> CertifyAsync(DocumentaryBlueprintCertificationRequest request, CancellationToken cancellationToken)
    {
        ...
        // Source lineage is the committed Phase 4 projection identity. Compatibility
        // artifacts retain their own metadata checksums and must never replace it.
        var longChecksum = aggregate.LongProjectionChecksum;
        var shortChecksum = aggregate.ShortProjectionChecksum;
        ...
        var transitions = new BlueprintTransitionReport(aggregate.ExecutionId, aggregate.PlanId, aggregate.EventId, aggregate.Language, aggregate.ProfileId,
            aggregate.AggregateId, aggregateChecksum, longChecksum, shortChecksum, version, transitionResults, transitionResults.All(x => x.IsValid), string.Empty);
        ...
        var pause = new BlueprintPauseTestReport(aggregate.ExecutionId, aggregate.PlanId, aggregate.EventId, aggregate.Language, aggregate.ProfileId,
            aggregate.AggregateId, aggregateChecksum, longChecksum, shortChecksum, version, pauseResults, pauseResults.Count(x => x.Passed), pauseResults.Count(x => !x.Passed), pauseResults.All(x => x.Passed), string.Empty);
        ...
        var sceneIntents = new BlueprintSceneIntentProjection(aggregate.ExecutionId, aggregate.PlanId, aggregate.EventId, aggregate.Language, aggregate.ProfileId,
            aggregate.AggregateId, aggregateChecksum, longChecksum, shortChecksum, version, intents, string.Empty);
        ...
        var validation = new BlueprintValidationReport(aggregate.ExecutionId, aggregate.PlanId, aggregate.EventId, aggregate.Language, aggregate.ProfileId,
            aggregate.AggregateId, aggregateChecksum, longChecksum, shortChecksum, version, validations,
