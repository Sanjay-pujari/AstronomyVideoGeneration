...
{
    private const string Schema = "phase14.scene-audio-sync/1.0";
    private const string SyncPolicy = "scene-level/1.0";
    private const string GroupingPolicy = "subtitle-text-8w-2l-42c/1.0";
    private static readonly JsonSerializerOptions Json = new(JsonSerializerDefaults.Web) { WriteIndented = true };

    public static async Task<IReadOnlyList<string>> ExecuteAsync(string root, string planId, string eventId, string language, CancellationToken ct)
    {
        ...
        foreach (var path in new[] { manifestPath, certificationPath, shortPath, longPath, p10Path })
        ...
        if (File.Exists(authorityPath))
        {
            ...
            if (old.RequestIdentity == identity && ValidateChecksum(old)) return Directory.EnumerateFiles(finalRoot).Append(Path.Combine(root, "validation", "phase-14-validation.json")).ToArray();
        }
        ...
        try
        {
            ...
            await Write(validationPath, new { schemaVersion = Schema, phaseNo = 14, phaseName = "Scene Audio Sync V1", status = "Succeeded", reasonCode = Phase14ReasonCodes.Accepted, ttsBoundaryModel = "SceneLevel", oneSceneOneAudioUnitPassed = true, subtitleSegmentsMayExceedAudioUnits = true, srtDefinesTtsBoundary = false, azureSpeechCalls = 0, otherTtsProviderCalls = 0, physicalAudioGenerated = false, semanticValidationPassed = true, checksumValidationPassed = true, manifestValidationPassed = true, publicationCommitted = true, committedStateValidationPassed = true, authorityChecksum = authority.AuthorityChecksum, downstreamReady = true }, ct);
            return Directory.EnumerateFiles(finalRoot).Append(validationPath).ToArray();
        }
    ...
    private static Phase14AudioSyncStream BuildStream(string format, string language, Phase7AcceptedReleaseCandidate candidate, IReadOnlyList<string> certifiedIds, string sourceChecksum, string sceneRef)
    {
        ...
        for (var i = 0; i < certifiedIds.Count; i++)
        {
            ...
            var sentences = Regex.Matches(text, @"[^.!?\u0964\u0965]+[.!?\u0964\u0965]*", RegexOptions.CultureInvariant).Select(m => Normalize(m.Value)).Where(x => x.Length > 0).ToArray();
            var ids = sentences.Select((_, n) => $"{format.ToLowerInvariant()}-{scene.SceneId}-sentence-{n + 1:D3}").ToArray();
            var unitId = $"sau-{Hash($"{format}|{scene.SceneId}|{language}|{i + 1}")[..20]}";
            var segments = SplitSubtitles(text).Select((chunk, n) => new SubtitleSegment($"sub-{Hash($"{unitId}|{n + 1}")[..20]}", n + 1, unitId, scene.SceneId,
                ids, chunk, Hash(chunk), Math.Max(800, chunk.Length * 70), chunk.Length <= 42 ? chunk : chunk[..Math.Min(42, chunk.Length)].TrimEnd(), chunk.Length <= 42 ? null : chunk[Math.Min(42, chunk.Length)..].Trim(), null, null, AudioSyncBreakReason.Sentence)).ToArray();
