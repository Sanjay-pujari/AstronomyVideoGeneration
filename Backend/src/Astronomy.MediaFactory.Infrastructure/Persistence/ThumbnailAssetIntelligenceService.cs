...
public sealed class ThumbnailAssetIntelligenceService(IOptions<RenderingOptions> renderingOptions, IOptions<AzureOpenAIForImageOptions> imageOptions, IVisualSourceResolver visualSourceResolver, IHttpClientFactory httpClientFactory, IOptions<ThumbnailOptions>? thumbnailOptions = null) : IThumbnailAssetIntelligenceService
{
    ...
    private const string ThumbnailV9ManifestFileName = "thumbnail-manifest.json";
    ...
    private async Task<ThumbnailAssetGenerationResponse> GenerateThumbnailCompositionModelAsync(ThumbnailAssetGenerationRequest request, CancellationToken cancellationToken)
    {
        ...
        var model = IsThumbnailV8Enabled(request)
    ...
    private async Task<ThumbnailAssetGenerationResponse> GenerateThumbnailSceneManifestAsync(ThumbnailAssetGenerationRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            manifest = IsThumbnailV8Enabled(request)
    ...
    private async Task<ThumbnailAssetGenerationResponse> GenerateThumbnailImagesAsync(ThumbnailAssetGenerationRequest request, CancellationToken cancellationToken)
    {
        ...
        var routed = await Phase12ThumbnailRouter.RouteAsync(
            thumbnailOptions?.Value,
            request,
            () => GenerateThumbnailV8AiNativeImagesAsync(request, thumbnailRoot, cancellationToken),
            () => GenerateThumbnailV7ImagesAsync(request, thumbnailRoot, cancellationToken),
            cancellationToken);
    ...
    private bool IsThumbnailDebugEnabled()
        => !string.Equals(thumbnailOptions?.Value.DebugLevel ?? "Off", "Off", StringComparison.OrdinalIgnoreCase);

    private string ResolveThumbnailDiagnosticsRoot(string thumbnailRoot)
    {
        if (!IsThumbnailDebugEnabled()) return thumbnailRoot;
        var debugRoot = Path.Combine(thumbnailRoot, "debug");
    ...
    private async Task<ThumbnailAssetGenerationResponse> GenerateThumbnailV8AiNativeImagesAsync(ThumbnailAssetGenerationRequest request, string thumbnailRoot, CancellationToken cancellationToken)
    {
        ...
        var promptAssemblyReportPath = Path.Combine(diagnosticsRoot, PromptAssemblyReportFileName);
        ...
        var compositionProfilePath = Path.Combine(diagnosticsRoot, ThumbnailCompositionProfileFileName);
        if (debugEnabled && false)
        ...
        var storytellingStrategyPath = Path.Combine(diagnosticsRoot, ThumbnailStorytellingStrategyFileName);
        if (debugEnabled && false)
        ...
        var visualDirectingProfilePath = Path.Combine(diagnosticsRoot, VisualDirectingProfileFileName);
        if (debugEnabled && false)
        ...
        var promptJsonPath = Path.Combine(diagnosticsRoot, ThumbnailPromptFileName);
        if (debugEnabled && false)
        ...
        var outputFileMap = outputPaths.ToDictionary(k => k.Key, v => v.Value);
        var visualPromptDiagnosticsPath = Path.Combine(diagnosticsRoot, VisualPromptDiagnosticsFileName);
        var promptDiffPath = Path.Combine(diagnosticsRoot, "thumbnail-prompt-diff.md");
        if (debugEnabled)
        {
        ...
        var diagnosticsPath = Path.Combine(diagnosticsRoot, ThumbnailGenerationDiagnosticsFileName);
        var v8DiagnosticsPath = Path.Combine(diagnosticsRoot, ThumbnailV8DiagnosticsFileName);
        var v8Diagnostics = new
        {
            ...
            aspectPromptsUnique = prompts.Select(prompt => prompt.Prompt).Distinct(StringComparer.OrdinalIgnoreCase).Count() == prompts.Length,
        ...
        if (debugEnabled && false)
            await WriteThumbnailV8Phase12ValidationAsync(diagnosticsRoot, allOutputs, promptValidation, cancellationToken);
        if (debugEnabled && false)
        ...
        if (debugEnabled)
            ValidateThumbnailDebugArtifacts(diagnosticsRoot);
        ...
        return BuildImageGenerationResponse(
            request,
            allOutputs.Concat(promptPaths.Values)
                .Concat(debugEnabled
                    ? new[] { NormalizePath(v8DiagnosticsPath), NormalizePath(contractJsonPath), NormalizePath(promptDiffPath), NormalizePath(visualPromptDiagnosticsPath) }
                    : Array.Empty<string>())
                .ToArray(),
            validation,
            ...
            thumbnailLayoutValidationPath: debugEnabled ? NormalizePath(v8DiagnosticsPath) : NormalizePath(Path.Combine(thumbnailRoot, ThumbnailV9ManifestFileName)));
    }
    ...
    private static void ValidateThumbnailDebugArtifacts(string debugRoot)
    {
        var requiredDebugArtifacts = new[]
        {
            ThumbnailV8DiagnosticsFileName,
            ThumbnailPromptContractFileName,
            "thumbnail-prompt-diff.md",
            VisualPromptDiagnosticsFileName
        };
        var missing = requiredDebugArtifacts
            .Select(fileName => Path.Combine(debugRoot, fileName))
            .Where(path => !File.Exists(path))
            .Select(NormalizePath)
            .ToArray();
        if (missing.Length > 0)
            throw new InvalidOperationException("Thumbnail debug diagnostics persistence validation failed: missing required artifact(s): " + string.Join(", ", missing));
    }
    ...
    private static object BuildThumbnailCompositionProfileDiagnostics(IReadOnlyList<ThumbnailPromptContract> contracts, IReadOnlyList<ThumbnailV8Prompt> prompts)
    {
        var entries = contracts.Select(contract =>
        {
            ...
            return new
            {
                ...
                platformTargets = profile.PlatformTargets,
                injectedCompositionGuidance = profile.CompositionGuidance,
                promptAdditions = profile.PromptAdditions,
        ...
        var promptTexts = prompts.Select(p => p.Prompt).ToArray();
        ...
        return new
        {
            ...
            validation = new
            {
                landscapePortraitSquarePromptsAreDifferent = promptTexts.Distinct(StringComparer.Ordinal).Count() == promptTexts.Length,
                compositionGuidanceIsNotReusedAcrossProfiles = profileGuidance.Distinct(StringComparer.OrdinalIgnoreCase).Count() == profileGuidance.Length,
                noPromptReusedAcrossAspectRatios = promptTexts.Distinct(StringComparer.Ordinal).Count() == promptTexts.Length,
                notes = new[]
                {
    ...
    {
        if (!Directory.Exists(thumbnailRoot)) return;
    ...
    {
        var manifestPath = Path.Combine(thumbnailRoot, ThumbnailV9ManifestFileName);
    ...
    private static class ThumbnailV8AiNativePromptBuilder
    {
        ...
        private static IThumbnailV8FamilyPromptBuilder SelectBuilder(ThumbnailV8Family family)
            => family switch
            {
                ...
                ThumbnailV8Family.Eclipse => new EclipseObservationGuidePromptBuilder(),
                _ => new PlanetaryObservationGuidePromptBuilder()
            };

        private enum ThumbnailV8Family { Planetary, Meteor, Moon, Eclipse }

        private static class ThumbnailFamilyResolver
        {
            public static ThumbnailV8Family Resolve(CurrentEventLock current)
            {
                var token = NormalizeEventTypeToken(current.EventType);
                if (token is "PLANETCONJUNCTION" or "PLANETGROUPING" or "PLANETALIGNMENT" or "PLANETPARADE" or "MOONPLANETCONJUNCTION" || IsPlanetaryEvent(current.EventType)) return ThumbnailV8Family.Planetary;
                if (token == "METEORSHOWER" || IsMeteorEvent(current.EventType, current.Title)) return ThumbnailV8Family.Meteor;
                if (token is "NAMEDFULLMOON" or "FULLMOON" or "SUPERMOON" or "BLUEMOON" or "WOLFMOON" or "STRAWBERRYMOON" or "HARVESTMOON" || IsMoonEvent(current.EventType, current.Title)) return ThumbnailV8Family.Moon;
                if (token is "SOLARECLIPSE" or "LUNARECLIPSE" or "ANNULARECLIPSE" or "PARTIALSOLARECLIPSE" or "PARTIALLUNARECLIPSE" || IsEclipseEvent(current.EventType, current.Title)) return ThumbnailV8Family.Eclipse;
                return ThumbnailV8Family.Planetary;
            }
        }
...
Recognizable realistic planetary disks with strong first-glance identity.
...
OBJECT PROMINENCE RULE: make the main celestial object or event object the strongest visual focus through camera framing, composition, perspective, and camera distance. Planets remain physically circular; never stretch, squash, elongate, or scale them non-uniformly.
