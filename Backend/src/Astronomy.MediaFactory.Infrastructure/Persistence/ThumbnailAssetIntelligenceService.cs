...
public sealed class ThumbnailAssetIntelligenceService(IOptions<RenderingOptions> renderingOptions, IOptions<AzureOpenAIForImageOptions> imageOptions, IVisualSourceResolver visualSourceResolver, IHttpClientFactory httpClientFactory) : IThumbnailAssetIntelligenceService
{
    ...
    private static ThumbnailOverlayDiagnostics DrawThumbnailV6SimpleOverlay(IImageProcessingContext ctx, CurrentEventLock current, int width, int height, string outputPath)
    {
        ...
        return new ThumbnailOverlayDiagnostics("ThumbnailV6Renderer", 3, false, false, false, false, false, outputPath, ResolveThumbnailV6Family(current));
    }
