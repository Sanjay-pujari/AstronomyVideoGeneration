    ...
    ILongSceneImageManifestValidator validator, IPhase9CommittedAuthorityReader reader) : ILongSceneImagePublicationService
{
    ...
    public async Task<LongSceneImagePublicationResult> PublishAsync(LongSceneImagePublicationRequest request, CancellationToken ct)
    {
        ...
        var sources = longSources.GroupBy(x => x.SceneId, StringComparer.Ordinal).ToDictionary(x => x.Key, x => x.First(), StringComparer.Ordinal);
        var expected = authority.LongScenes.OrderBy(x => x.SceneOrder).ToArray();
        if (longSources.Length != sources.Count || sources.Count != expected.Length || expected.Any(x => !sources.ContainsKey(x.SceneId))) Fail(Phase9ReasonCodes.SceneSetMismatch, "Phase 8 Long set does not exactly match Phase 6.");
        ...
        try
        {
            ...
            foreach (var frame in expected)
            {
                ...
                if (!source.StartsWith(phase8Root, StringComparison.Ordinal) || !File.Exists(source)) Fail(Phase9ReasonCodes.SourceMissing, $"Source image is missing for '{frame.SceneId}'.");
                ...
                if (info is null || info.Width != asset.Width || info.Height != asset.Height || asset.AspectRatio != $"{info.Width}:{info.Height}") Fail(Phase9ReasonCodes.SourceDimensionMismatch, $"Source image dimensions differ for '{frame.SceneId}'.");
                ...
                if (asset.RequiresScientificGeometry && (!asset.ScientificGeometryCertified || string.IsNullOrWhiteSpace(asset.AccuracyEvidencePath) || !File.Exists(Path.Combine(request.OutputRoot, asset.AccuracyEvidencePath)))) Fail(Phase9ReasonCodes.ScientificEvidenceInvalid, $"Scientific evidence is invalid for '{frame.SceneId}'.");
                ...
                await using var copied = File.OpenRead(target); var copiedHash = Convert.ToHexString(await SHA256.HashDataAsync(copied, ct)).ToLowerInvariant();
                if (!copiedHash.Equals(hash, StringComparison.OrdinalIgnoreCase)) Fail(Phase9ReasonCodes.SourceChecksumMismatch, $"Materialized checksum differs for '{frame.SceneId}'.");
                items.Add(new($"P9:{frame.SceneId}", frame.SceneId, frame.BlueprintSceneId, frame.StoryFrameId, frame.SceneOrder, asset.AssetId, asset.SemanticIdentity,
                    ...
                    asset.AspectRatio, copiedHash, true, false, false, asset.RequiresScientificGeometry, asset.ScientificGeometryCertified, asset.AccuracyEvidencePath, "Valid", []));
            }
            ...
            var candidateReadback = JsonSerializer.Deserialize<LongSceneImageManifest>(await File.ReadAllTextAsync(Path.Combine(staging, "long-scene-image-manifest.json"), ct), JsonOptions);
            var candidateReadbackValidation = candidateReadback is null ? new(false, [Phase9ReasonCodes.SourceInvalid], ["Candidate manifest missing."]) : await validator.ValidateAsync(candidateReadback, phase8, authority, staging, ct);
            if (!candidateReadbackValidation.IsValid) throw new Phase9AuthorityException(candidateReadbackValidation.ReasonCodes.First(), string.Join("; ", candidateReadbackValidation.Errors));
            var diagnostics = new { phase9Applicable=true, requestedLong=true, phase8AuthorityLoaded=true, phase8AuthorityChecksum=phase8.DeterministicChecksum, phase8Committed=true, phase8DownstreamReady=true, phase6AuthorityLoaded=true, phase6StoryFrameManifestChecksum=authority.StoryFrameManifestChecksum, expectedLongSceneCount=expected.Length, phase8LongAssetCount=sources.Count, materializedLongSceneCount=items.Count, missingSceneIds=Array.Empty<string>(), extraSceneIds=Array.Empty<string>(), lineageMismatchSceneIds=Array.Empty<string>(), sourceChecksumMismatchSceneIds=Array.Empty<string>(), dimensionMismatchSceneIds=Array.Empty<string>(), scientificEvidenceFailureSceneIds=Array.Empty<string>(), cinematicAssetCount=items.Count(x=>x.VisualStyle=="Cinematic"), hybridCinematicAssetCount=items.Count(x=>x.VisualStyle=="HybridCinematic"), explicitInfographicAssetCount=items.Count(x=>x.VisualStyle=="Infographic"), azureCallsThisPhase=0, sceneAssetsV3GenerationCallsThisPhase=0, infographicRendererCallsThisPhase=0, candidateValidationPassed=true, candidateReadbackPassed=true, publicationCommitted=true, committedReadbackPassed=true, downstreamReady=true, legacyNineSceneContractUsed=false, upstreamArtifactsModified=false };
