...
public sealed class WeeklySkyForecastV2CinematicEditorialRefiner : IWeeklySkyForecastV2CinematicEditorialRefiner
{
    ...
    public Task<WeeklyCinematicStoryBlueprint> RefineAsync(WeeklyEditorialStoryPackage editorialPackage, WeeklySkyForecastV2IntelligenceResponse intelligence, CancellationToken cancellationToken)
    {
        ...
        var editorialPeakDate = intelligence.SkyfieldSummary.BestPhotographyNight ?? intelligence.SkyfieldSummary.BestMoonNight ?? groupedEvents?.OrderByDescending(e => e.VisualScore + e.StoryScore).FirstOrDefault()?.PrimaryDate ?? heroSource.PrimaryDate;
        ...
        var openingHook = "This week’s evening sky creates one simple reason to look west after sunset.";
        ...
        {
            new("hero_grouping","Hero grouping visual",heroStory.Description,heroObjects,editorialPeakDate,heroSource.BestTimeUtc,"wide",heroSource.RecommendedVisualStrategy,"hero_grouping",false,$"grouping:{string.Join('-', heroObjects.OrderBy(x=>x))}"),
            ...
            new("thumbnail_hero","Thumbnail composition","Primary thumbnail framing for this week.",heroObjects,editorialPeakDate,heroSource.BestTimeUtc,"thumbnail","Hybrid","thumbnail",true,"thumbnail:hero-grouping")
        };
