...
public sealed class WeeklyAstronomyEventExtractor : IWeeklyAstronomyEventExtractor
{
    public WeeklyAstronomyEventExtractionResult Extract(WeeklySkyForecastContext context, string region, DateOnly weekStartDate, DateOnly weekEndDate, string language, string? workingDirectoryRoot)
    {
        ...
        {
            var best = obj.OrderByDescending(o => o.VisibilityScore).ThenByDescending(o => o.AltitudeDegrees ?? 0).First();
            events.Add(BuildEvent(WeeklyAstronomyEventType.HeroObject, $"{best.ObjectName} this week", $"{best.ObjectName} is a strong standalone viewing target this week.", [best], best.ObjectCode, best.BestViewingTimeUtc, best.Direction, null, "StellariumScene", "HeroObjectCloseup", "Object-focused narration angle"));
        }
        ...
        foreach (var day in context.DailyForecasts)
        {
            ...
            if (grouping.Count >= 2)
            {
                events.Add(BuildEvent(grouping.Count >= 3 ? WeeklyAstronomyEventType.Grouping : WeeklyAstronomyEventType.Conjunction,
                    ...
                    grouping, grouping.First().ObjectCode, grouping.First().BestViewingTimeUtc, grouping.First().Direction, 12, "StellariumScene", grouping.Count >=3 ? "MultiObjectSkyGrouping" : "ConjunctionScene", "Comparative visual narration"));
            }
        }
        ...
        if (bestNight is not null)
        {
            ...
            events.Add(BuildEvent(WeeklyAstronomyEventType.BestViewingWindow, "Best viewing window", "Best local viewing window for the week with practical observing convenience.", dayObjects, dayObjects.FirstOrDefault()?.ObjectCode, bestNight.BestStartUtc, dayObjects.FirstOrDefault()?.Direction, null, "StellariumScene", "BestWindowScene", "Actionable viewing recommendation"));
            events.Add(BuildEvent(WeeklyAstronomyEventType.DirectionalObservation, $"Look {dayObjects.FirstOrDefault()?.Direction ?? "west"} after sunset", "Use horizon direction guidance and altitude to find the best objects.", dayObjects, dayObjects.FirstOrDefault()?.ObjectCode, bestNight.BestStartUtc, dayObjects.FirstOrDefault()?.Direction, null, "StellariumScene", "DirectionalGuideScene", "Step-by-step finder guidance"));
        }
    ...
    private static WeeklyAstronomyEvent BuildEvent(WeeklyAstronomyEventType type, string title, string summary, IReadOnlyList<VisibleSkyObject> objs, string? primary, DateTime? bestTimeUtc, string? direction, double? separation, string visualSource, string sceneType, string narrationAngle)
    {
        var first = objs.FirstOrDefault();
        var objects = objs.Select(o => new WeeklyAstronomyEventObject(o.ObjectCode, o.ObjectName, o.AltitudeDegrees, o.AzimuthDegrees, o.Magnitude, o.VisibilityScore)).ToList();
        return new WeeklyAstronomyEvent(Guid.NewGuid().ToString("N"), type, title, summary, objects, primary, objects.Count, bestTimeUtc.HasValue ? DateOnly.FromDateTime(bestTimeUtc.Value) : null, bestTimeUtc.HasValue ? TimeOnly.FromDateTime(bestTimeUtc.Value.ToLocalTime()) : null, direction, first?.AltitudeDegrees, first?.AzimuthDegrees, separation, first?.Magnitude, objs.DefaultIfEmpty().Average(o => o?.VisibilityScore ?? 0), Math.Min(100, 60 + objects.Count * 10), type is WeeklyAstronomyEventType.Grouping or WeeklyAstronomyEventType.Conjunction ? 70 : 45, visualSource, sceneType, narrationAngle, []);
    }
}
