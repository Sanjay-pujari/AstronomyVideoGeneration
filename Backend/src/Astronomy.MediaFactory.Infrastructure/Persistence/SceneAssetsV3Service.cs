    ...
    ILogger<SceneAssetsV3Service> logger) : ISceneAssetsV3Service
{
    ...
    private async Task WriteAuthorityPlanningPackageAsync(SceneAssetsV3Request request, Phase8AuthorityInput authority, CancellationToken ct)
    {
        ...
        await WriteJsonAsync(Path.Combine(root, "media-project.json"), new
        {
            ...
            shortProfile = new { targetWidth = request.ShortTargetWidth, targetHeight = request.ShortTargetHeight,
                targetAspectRatio = $"{request.ShortTargetWidth}:{request.ShortTargetHeight}" },
            longProfile = new { targetWidth = request.LongTargetWidth, targetHeight = request.LongTargetHeight,
                targetAspectRatio = $"{request.LongTargetWidth}:{request.LongTargetHeight}" },
...
""";
        ...
        Phase8AuthorityInput authority, string compatibilityRoot, IReadOnlySet<string> reusableAssets, CancellationToken ct)
    {
        ...
        try
        {
            ...
            await WriteJsonAsync(Path.Combine(staging, "phase8-authority-diagnostics.json"), new { authorityLoaded = true, phase4Committed = true, phase6Committed = true, requestedShort = authority.RequestedVariants.Contains("Short"), requestedLong = authority.RequestedVariants.Contains("Long"), longNarrationCandidateCommitted = authority.LongNarrationReleaseCandidate is not null, shortNarrationCandidateCommitted = authority.ShortNarrationReleaseCandidate is not null, authority.RequestedVariants, expectedLongSceneCount = authority.LongScenes.Count, expectedShortSceneCount = authority.ShortScenes.Count, generatedLongSceneCount = items.Count(x => x.Variant == "Long"), generatedShortSceneCount = items.Count(x => x.Variant == "Short"), reusedAssetCount = items.Count(x => x.Reused), generatedAssetCount = items.Count(x => !x.Reused), azureGeneratedShortCount = items.Count(x => x.Variant == "Short" && x.ImageGenerationProvider == "AzureOpenAICinematicImageGenerator"), azureGeneratedLongCount = items.Count(x => x.Variant == "Long" && x.ImageGenerationProvider == "AzureOpenAICinematicImageGenerator"), hybridShortCount = items.Count(x => x.Variant == "Short" && x.VisualStyle == nameof(Phase8VisualStyle.HybridCinematic)), hybridLongCount = items.Count(x => x.Variant == "Long" && x.VisualStyle == nameof(Phase8VisualStyle.HybridCinematic)), explicitInfographicShortCount = items.Count(x => x.Variant == "Short" && x.VisualStyle is nameof(Phase8VisualStyle.Infographic) or nameof(Phase8VisualStyle.ScientificChart)), explicitInfographicLongCount = items.Count(x => x.Variant == "Long" && x.VisualStyle is nameof(Phase8VisualStyle.Infographic) or nameof(Phase8VisualStyle.ScientificChart)), azureCallsCount = items.Count(x => x.ProviderCalledThisExecution), fallbackAssetCount = items.Count(x => x.ProviderStatus.Contains("Fallback", StringComparison.Ordinal)), providerTypeCounts = items.GroupBy(x => x.ProviderType).ToDictionary(x => x.Key, x => x.Count()), missingShortSceneIds = Array.Empty<string>(), missingLongSceneIds = Array.Empty<string>(), extraSceneIds = Array.Empty<string>(), lineageMismatchSceneIds = Array.Empty<string>(), upstreamChecksumPassed = true, manifestValidationPassed = true, candidateReadbackPassed = true, publicationCommitted = true, committedReadbackPassed = true, downstreamReady = true, legacyNineSceneContractUsed = false }, ct);
