    ...
    INarrationGenerationService? narrationGenerationService = null) : IProductionPipelineExecutionService, IProductionPhaseRunner
{
    ...
    private static void UpdateThumbnailV8StepDiagnostic(JsonObject step, ThumbnailAssetGenerationResponse response, bool v8Enabled)
    {
        ...
        var v8GeneratorCalled = v8Enabled && string.Equals(response.PhaseRequested, "Images", StringComparison.OrdinalIgnoreCase) &&
            (selectedRenderer.Contains("V9", StringComparison.OrdinalIgnoreCase) || response.GeneratedFiles.Any(file => file.Contains("thumbnail-v9-diagnostics", StringComparison.OrdinalIgnoreCase)));
        ...
        step["skippedDueToRendererSelection"] = v8Enabled && string.Equals(response.PhaseRequested, "Images", StringComparison.OrdinalIgnoreCase) && !selectedRenderer.Contains("V9", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static HeroLayoutValidationSummary SummarizeHeroLayoutValidation(string layoutValidationPath)
    {
        ...
        var objectVisibilityPass = ObjectVisibilityAllVisibleAndUncropped(root);
        ...
        bool? heroTitleSafeAreaPassed = null;
        bool? safeArea = null;

        if (heroOverlayDiagnostics.ValueKind == JsonValueKind.Object)
        {
            ...
            heroTitleSafeAreaPassed = ReadNullableBool(heroOverlayDiagnostics, "heroTitleSafeAreaPassed");
    ...
    {
        if (!root.TryGetProperty("objectVisibility", out var objects) || objects.ValueKind != JsonValueKind.Array)
            return true;
        ...
        foreach (var item in objects.EnumerateArray())
        {
            if (item.ValueKind != JsonValueKind.Object)
                return false;
            if (ReadNullableBool(item, "visible") == false || ReadNullableBool(item, "cropped") == true)
                return false;
        }
    ...
    private static HeroOverlayTextValidationDiagnostics ValidateCinematicHeroOverlayTextWithRenderedLayout(HeroOverlayRole role, string hookText, string visibleText = "", string eventFamily = "", int maxWords = DefaultMaxHeroOverlayWords, bool renderedLayoutFits = false)
    {
        ...
        var finalDecisionFalseLine = 0;
        var emptyHookText = TraceHeroValidationBool(string.IsNullOrWhiteSpace(normalizedHookText), "string.IsNullOrWhiteSpace(normalizedHookText)");
        var duplicatedOverlayText = TraceHeroValidationBool(HasDuplicatedOverlayText(normalizedHookText, normalizedVisibleText), "HasDuplicatedOverlayText(normalizedHookText, normalizedVisibleText)");
        var narrationSentenceLike = TraceHeroValidationBool(isNarrationRole && isSentenceLike, "isNarrationRole && isSentenceLike");
        var safeAreaFailed = TraceHeroValidationBool(!fitsSafeArea, "!fitsSafeArea");
        var renderedLayoutFailed = TraceHeroValidationBool(!isNarrationRole && !renderedLayoutFits, "!isNarrationRole && !renderedLayoutFits");
        var hookValidationPassed = !emptyHookText && !duplicatedOverlayText && !narrationSentenceLike && !safeAreaFailed;
        var renderedLayoutPassed = !renderedLayoutFailed;
        TraceHeroValidation($"HeroOverlayTextValidator TRACE_STATE renderedLayoutPassed={renderedLayoutPassed}; hookValidationPassed={hookValidationPassed}; fitsSafeArea={fitsSafeArea}; isSentenceLike={isSentenceLike}; wordCount={wordCount}; eventFamily='{(string.IsNullOrWhiteSpace(eventFamily) ? "Unknown" : eventFamily)}'");
    ...
    private static string ResolveHeroValidationContract(string blueprintPath, string layoutValidationPath, string rendererContract)
    {
        ...
        return string.Equals(rendererContract, "CinematicHero", StringComparison.OrdinalIgnoreCase) ? "CinematicHero" : "GuideHero";
    }
    ...
    private static string ReadExplicitHeroContractFromLayoutValidation(string layoutValidationPath)
    {
        ...
        using var doc = JsonDocument.Parse(File.ReadAllText(layoutValidationPath));
        var root = doc.RootElement;
        foreach (var name in new[] { "explicitHeroContract", "requestedHeroContract" })
            if (root.TryGetProperty(name, out var explicitProp) && explicitProp.ValueKind == JsonValueKind.String && !string.IsNullOrWhiteSpace(explicitProp.GetString()))
                return explicitProp.GetString()!;
        foreach (var name in new[] { "heroContract", "validatorContract", "validationProfileUsed" })
            if (root.TryGetProperty(name, out var prop) && prop.ValueKind == JsonValueKind.String && !string.IsNullOrWhiteSpace(prop.GetString()))
            {
                ...
                if (string.Equals(value, "CinematicHero", StringComparison.OrdinalIgnoreCase) && ContainsGuideHeroBlocks(ReadRenderedBlocks(layoutValidationPath)))
                    continue;
                return value;
            }
        return string.Empty;
    ...
    private static void ValidateThumbnailV8Contract(string thumbnailRoot)
    {
        ...
        using (var phase12 = JsonDocument.Parse(File.ReadAllText(phase12ValidationPath)))
        {
            ...
            if (!string.Equals(GetJsonString(phase12Root, "thumbnailVersion", string.Empty), "V9", StringComparison.Ordinal)
                || !string.Equals(GetJsonString(phase12Root, "selectedRenderer", string.Empty), "ThumbnailV9AiFinalThumbnailComposer", StringComparison.Ordinal))
                throw new InvalidOperationException("Thumbnail V9 validation failed: phase-12-validation.json must report V9 and ThumbnailV9AiFinalThumbnailComposer.");
            if (File.ReadAllText(phase12ValidationPath).Contains("V7", StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException("Thumbnail V9 validation failed: V7 appeared in phase-12-validation.json while V9 is enabled.");
        }
        ...
        if (legacy.Length > 0)
            throw new InvalidOperationException("Thumbnail V9 validation failed: duplicate non-thumbnail-prefixed output(s) generated: " + string.Join(", ", legacy));
    ...
    private static Phase12ThumbnailDiagnostics BuildPhase12ThumbnailV8Diagnostics(ProductionPhaseContext context)
    {
        ...
        return new Phase12ThumbnailDiagnostics(
            ...
            ValidatorProfile: "PromptValidatorV9",
            MoonPhaseName: string.Empty,
            MoonIlluminationPercent: string.Empty,
            MoonriseLocal: string.Empty,
            MoonsetLocal: string.Empty,
            ...
            MoonForbiddenTermsDetected: Array.Empty<string>(),
            ThumbnailVersion: "V9",
            ThumbnailContract: "ThumbnailV9AiFinal",
            Renderer: "ThumbnailV9AiFinalThumbnailComposer",
            Validator: "PromptValidatorV9",
            ThumbnailReviewJsonRequired: false,
            V6RendererExecuted: false,
    ...
    private static Phase11HeroDiagnostics BuildPhase11HeroDiagnostics(ProductionPhaseContext context)
    {
        ...
        var validatorContract = ContainsGuideHeroBlocks(renderedBlocks) ? "GuideHero" : FirstNonEmpty(ReadHeroContractDiagnosticValue(layoutValidationPath, "validatorContract"), heroContract);
        var validationProfileUsed = ContainsGuideHeroBlocks(renderedBlocks) ? "GuideHero" : FirstNonEmpty(ReadHeroContractDiagnosticValue(layoutValidationPath, "validationProfileUsed"), validatorContract);
        var contractMismatch = !string.IsNullOrWhiteSpace(rendererContract) && !string.Equals(rendererContract, validatorContract, StringComparison.OrdinalIgnoreCase);
        var forbiddenBlocks = string.Equals(validatorContract, "CinematicHero", StringComparison.OrdinalIgnoreCase)
