...
public sealed class Rc2PublishingPublicationConfiguration : IEntityTypeConfiguration<Rc2PublishingPublication>
{
    public void Configure(EntityTypeBuilder<Rc2PublishingPublication> entity)
    {
        ...
        // Provider diagnostics are variable length. Their application-level size and redaction
        // policy is enforced before persistence; the database must not mask a provider failure.
        entity.Property(x => x.FailureMessage).HasColumnType("text");
