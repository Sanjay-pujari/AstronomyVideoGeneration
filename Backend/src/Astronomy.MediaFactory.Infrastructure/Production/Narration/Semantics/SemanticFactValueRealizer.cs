...
public sealed class SemanticFactValueRealizer : ISemanticFactValueRealizer
{
    ...
    {
        if (value is Sources.Adapters.Contracts.CanonicalAstronomyEventIdentity eventIdentity) return (First(eventIdentity.DisplayName, eventIdentity.ShortTitle, eventIdentity.Title, eventIdentity.CanonicalEventType, eventIdentity.FamilyId, eventIdentity.SourceEventType), "EventIdentity", "TypedValueRealizer");
        ...
        if (value is DomainScientificKnowledgeValue dk) return (First(dk.ScientificSignificance, dk.Mechanism, dk.PerspectiveAlignmentExplanation, dk.StableObservingPrinciples), "DomainScientificKnowledge", "TypedValueRealizer");
        if (value is MeteorActivityValue m) return (string.Join(", ", new[] { string.IsNullOrWhiteSpace(m.ShowerName) ? null : m.ShowerName, m.Zhr.HasValue ? $"ZHR {m.Zhr.Value.ToString(CultureInfo.InvariantCulture)}" : null, string.IsNullOrWhiteSpace(First(m.Radiant, m.RadiantConstellation)) ? null : $"radiant near {First(m.Radiant, m.RadiantConstellation)}", m.VelocityKmS.HasValue ? $"{m.VelocityKmS.Value:0.#} km/s" : null, m.PeakWindow?.LocalizedWindowDescription, m.MoonInterference, m.VisibilityNotes, m.MechanicsExplanation }.Where(x => !string.IsNullOrWhiteSpace(x))), "MeteorActivity", "TypedValueRealizer");
