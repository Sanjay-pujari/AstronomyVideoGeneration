...
public sealed class SemanticCapabilityCatalog : ISemanticCapabilityCatalog
{
    ...
    private static IEnumerable<SemanticCapabilityDefinition> Build()
    {
        ...
        yield return Cap("AstronomicalObjects", ["AstronomicalObjects","PrimaryObjects","OccultingObject","HiddenObject","ObjectName","Name","Objects"], 80, ["ProductionEventIntelligencePrimaryObjectsAdapter","EditorialContractPrimaryObjectsAdapter","DocumentaryContractPrimaryObjectsAdapter"]);
        yield return Cap("SecondaryAstronomicalObjects", ["SecondaryAstronomicalObjects","SecondaryObjects","HiddenObject","OccultingObject"], 70, ["ProductionEventIntelligencePrimaryObjectsAdapter","EditorialContractPrimaryObjectsAdapter","DocumentaryContractPrimaryObjectsAdapter"]);
        yield return Cap("EventWindow", ["EventWindow","EventDate","EventDateOrWindow","Date","ViewingWindow","PeakWindow","StartTime","EndTime"], 80, ["ProductionEventIntelligenceEventDateAdapter","ObservationMetadataEventDateAdapter"], rules:["EventDateOrWindowCompatibilityProjection"]);
        ...
        yield return Cap("ObservationEquipment", ["ObservationEquipment","ObservationMode","VisibilityMethod","NakedEye","BinocularGuidance","TelescopeGuidance"], 70, ["VisibilityMethodAdapter"]);
        ...
        yield return Cap("ObservationConditions", ["ObservationConditions","VisibilityConditions","DarkSkyGuidance","MoonPhase","Visibility"], 60, ["VisibilityMethodAdapter"]);
        yield return Cap("MeteorActivity", ["MeteorActivity","Zhr","ZHR","ZenithalHourlyRate","Zenithal Hourly Rate","Radiant","PeakWindow"], 75, ["ProductionEventIntelligenceMeteorActivityAdapter","ProductionEventIntelligenceZhrAdapter"], strictness: "OptionalEventSpecific", eventSpecific: true);
        yield return Cap("EclipseCircumstances", ["EclipseCircumstances","EclipseType","Magnitude","Contacts","ContactTimes"], 80, ["EclipseCircumstancesAdapter"], rules:["StructuredEclipseMetadataProjection"], eventSpecific: true);
        yield return Cap("ObjectKnowledge", ["ObjectKnowledge","ScientificIdentity","IdentificationPattern","SkyRegion","MajorStars","Name","ObjectName","ObjectType","ScientificImportance"], 70, ["ObjectKnowledgeStructuredAdapter","DocumentaryContractPrimaryObjectsAdapter","EditorialContractPrimaryObjectsAdapter"], domain:["ObjectKnowledge","ScientificIdentity","ScientificImportance"]);
        yield return Cap("DomainScientificKnowledge", ["DomainScientificKnowledge","ScientificMechanism","ScientificSignificance","ScientificImportance","Mechanism","ApparentPairingScience","ApparentAlignmentExplanation"], 80, ["DomainScientificKnowledgeAdapter","DomainKnowledgeApparentAlignmentAdapter"], rules:["ReviewedScientificKnowledge","ApprovedDerivationRules"], domain:["DomainScientificKnowledge","ScientificMechanism","ScientificSignificance","ApparentAlignmentExplanation"]);
        yield return Cap("EditorialContext", ["EditorialContext","EditorialIntent","Do not force event structure","No raw producer notes"], 60, ["EditorialContextAdapter"], domain:["EditorialContext"], strictness: "OptionalStructuredKnowledge");
        yield return Cap("CulturalNameContext", ["CulturalNameContext","GreekMythology","Greek mythology","HunterOrion","Hunter Orion","TraditionalSkyCulture","HistoricalNaming","RegionalCulturalNotes"], 60, ["CulturalNameContextStructuredKnowledgeAdapter"], domain:["GreekMythology","HunterOrion","TraditionalSkyCulture","HistoricalNaming","RegionalCulturalNotes"], strictness: "OptionalStructuredKnowledge");
