...
public sealed class AstronomyFamilyProfileV1CompatibilityAdapter : IAstronomyFamilyProfileV1CompatibilityAdapter
{
    ...
    private static FamilyProfileCompatibilityMapping Map(AstronomyFamilyProfileV1 profile, FamilySemanticRequirementV1 requirement, bool required)
    {
        ...
        var legacy = (family, cap) switch
        {
            ...
            ("MeteorShower", SemanticCapabilityVocabularyV1.MeteorActivity) => [SemanticCapabilityVocabularyV1.MeteorActivity],
            ...
            ("SolarEclipse" or "LunarEclipse", SemanticCapabilityVocabularyV1.EclipseCircumstances) => [SemanticCapabilityVocabularyV1.EclipseCircumstances],
    ...
    private static AstronomyFamilyProfile Legacy(AstronomyFamilyProfileV1 p, IReadOnlyList<string> req, IReadOnlyList<string> opt) => new(
        ...
        p.LongFormStructure.Beats.Select(b => b.BeatRole).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(), p.LongFormStructure.Beats.OrderBy(b => b.Order).Select(b => b.BeatRole).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(),
        p.Policy.EventSpecificTimingRequired ? "Use verified observing details only." : "Observation details optional.", p.Policy.EventSpecificTimingRequired ? "Event date or window is required." : "No event date required.", ["V1Compatibility"], ["Unverified runtime facts"], ["Compatibility adapter must not invent semantic values"]);
}
