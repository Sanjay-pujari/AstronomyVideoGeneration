...
public sealed class Phase7NarrationDraftInputAuthorityEvaluator(
    IPhase7NarrationPlanningCommittedStateEvaluator committedEvaluator, IFamilyNarrationProfileResolver profiles)
    : IPhase7NarrationDraftInputAuthorityEvaluator
{
    public async Task<Phase7NarrationDraftInputAuthorityEvaluation> EvaluateAsync(Phase7NarrationDraftInputAuthorityRequest request,CancellationToken token=default)
    {
        ...
        if(!string.Equals(a.Language,r.Language,StringComparison.OrdinalIgnoreCase))
            return Fail(NarrationDraftReasonCodes.LanguageMismatch,["Planning language does not match the request."],result.Warnings);
        var profile=profiles.Profiles.SingleOrDefault(x=>x.ProfileId==a.ProfileId&&x.ContractVersion==a.ProfileVersion);
        if(profile is null) return Fail(NarrationDraftReasonCodes.ProfileMismatch,["The committed family profile is unavailable."],result.Warnings);
        ...
        var input=new Phase7NarrationDraftInputAuthority(p,a,p.Diagnostics,p.Report,p.Validation,p.ManifestEntry,p.PublicationEvidence,
            ...
            {CertifiedClaims=request.CertifiedClaims,Warnings=result.Warnings};
        return new(true,input,NarrationDraftReasonCodes.InputValid,[],result.Warnings);
    }
    private static Phase7NarrationDraftInputAuthorityEvaluation Fail(string code,IReadOnlyList<string> errors,IReadOnlyList<string> warnings)=>new(false,null,code,errors,warnings);
}
...
{
    public bool Supports(string x)=>x.StartsWith("en",StringComparison.OrdinalIgnoreCase)||x.StartsWith("hi",StringComparison.OrdinalIgnoreCase);
    public string Terminate(string text,string language){text=text.Trim();if(text.Length==0)return text;return text.EndsWith('.')||text.EndsWith('?')||text.EndsWith('!')||text.EndsWith('।')?text:text+(language.StartsWith("hi",StringComparison.OrdinalIgnoreCase)?"।":".");}
    ...
    public string Conjunction(string language)=>language.StartsWith("hi",StringComparison.OrdinalIgnoreCase)?" और ":" and ";
    public decimal EstimateReadingTime(string text,string language)=>Math.Round(Count(text)/(language.StartsWith("hi",StringComparison.OrdinalIgnoreCase)?130m:150m)*60m,3);
...
{
    public NarrationDraftTimingBudget Resolve(NarrationDraftTimingRequest r){var wpm=r.Language.StartsWith("hi",StringComparison.OrdinalIgnoreCase)?130m:150m;var min=(int)Math.Floor(r.MinimumDurationSeconds*wpm/60m);var max=(int)Math.Ceiling(r.MaximumDurationSeconds*wpm/60m);var target=(int)Math.Round(r.TargetDurationSeconds*wpm/60m);return new(target,min,max,r.TargetDurationSeconds,1m,Math.Max(0,r.MaximumSentenceCount-r.RequiredClaimCount-1),wpm);}
}
    ...
    INarrationDraftTransitionPhrasePolicy transitions,INarrationDraftSafetyValidator safety):INarrationDraftAuthorityBuilder
{
    ...
    private (NarrationDraftScene? Scene,NarrationDraftAuthorityBuildResult? Error) BuildScene(NarrationPlanningScene p,bool hasNext,IReadOnlyDictionary<string,CertifiedNarrationClaim> catalog,string lang)
    {
        ...
        if(required.Concat(p.OptionalClaims.Where(catalog.ContainsKey).Select(x=>catalog[x])).Any(x=>x.RequiresHumanReview))return(null,Fail(NarrationDraftReasonCodes.HumanReviewInvalid,"A factual claim requires human review."));
        ...
        foreach(var c in required){var q=Qualifications(p,c);var text=realization.Realize(c,q,lang);if(!language.PreservesProtectedTokens(c.Text,text))return(null,Fail(NarrationDraftReasonCodes.SafetyInvalid,$"Protected numeric, unit, name, date, or direction token changed: {c.ClaimId}"));var s=AddSentence(text,"RequiredClaim",[c.ClaimId],q,true,false,false,c.KnowledgeReferenceIds);usages.Add(Usage(c.ClaimId,"Required",s,q));}
        var optionalUsages=new List<NarrationDraftClaimUsage>();foreach(var id in p.OptionalClaims.Take(budget.PermittedOptionalClaimCapacity)){if(!catalog.TryGetValue(id,out var c)||!string.Equals(c.Language,lang,StringComparison.OrdinalIgnoreCase))continue;var q=Qualifications(p,c);var text=realization.Realize(c,q,lang);if(sentences.Sum(x=>DeterministicNarrationDraftLanguagePolicy.Count(x.Text))+DeterministicNarrationDraftLanguagePolicy.Count(text)>budget.MaximumWords)continue;var s=AddSentence(text,"OptionalClaim",[id],q,false,true,false,c.KnowledgeReferenceIds);optionalUsages.Add(Usage(id,"Optional",s,q));}
        ...
        var incoming=transitions.Create(p.IncomingTransition,lang);var outgoing=transitions.Create(p.OutgoingTransition,lang);
        var draft=new NarrationDraftScene("",p.PlanningId,p.SceneId,p.Variant,p.StoryFrameId,p.PacketId,p.DeterministicChecksum,p.ViewerQuestion,p.LearningObjective,sentences[0].Text,sentences,
            closings.Create(p,hasNext,lang),incoming,outgoing,usages,optionalUsages,p.DeferredClaims,usages.Concat(optionalUsages).SelectMany(x=>x.QualificationIds).Distinct().ToArray(),p.SafetyRequirements,p.EditorialConstraints,words,sentences.Count,read,p.MinimumDuration,p.ExpectedDuration,p.MaximumDuration,"");
