    ...
    INarrationDraftTransitionPhrasePolicy transitions,INarrationDraftSafetyValidator safety):INarrationDraftAuthorityBuilder
{
    ...
    {var result=new List<NarrationDraftScene>();for(var i=0;i<source.Count;i++){var built=BuildScene(source[i],i+1,i+1<source.Count,claims,lang);if(built.Error is not null)return(null,built.Error);result.Add(built.Scene!);}return(result,null);}
    private (NarrationDraftScene? Scene,NarrationDraftAuthorityBuildResult? Error) BuildScene(NarrationPlanningScene p,int sceneNumber,bool hasNext,IReadOnlyDictionary<string,CertifiedNarrationClaim> catalog,string lang)
    {
        ...
        var completeSceneSentences=sentences.ToArray();
        var words=completeSceneSentences.Sum(x=>DeterministicNarrationDraftLanguagePolicy.Count(x.Text));var read=completeSceneSentences.Sum(x=>x.EstimatedDurationSeconds);
        if(completeSceneSentences.Length<decision.MinimumSentenceCount)return(null,TimingFail(NarrationDraftReasonCodes.SceneSentenceCountBelowMinimum,"SceneSentenceRangeGate",p,sceneNumber,completeSceneSentences,decision,read,usages,optionalUsages,$"Scene {p.Variant}/{p.SceneId} has {completeSceneSentences.Length} complete spoken sentences; minimum is {decision.MinimumSentenceCount}."));
        if(completeSceneSentences.Length>decision.MaximumSentenceCount)return(null,TimingFail(NarrationDraftReasonCodes.SceneSentenceCountAboveMaximum,"SceneSentenceRangeGate",p,sceneNumber,completeSceneSentences,decision,read,usages,optionalUsages,$"Scene {p.Variant}/{p.SceneId} has {completeSceneSentences.Length} complete spoken sentences; maximum is {decision.MaximumSentenceCount}."));
        if(read<decision.MinimumDurationSeconds)return(null,TimingFail(NarrationDraftReasonCodes.SceneDurationBelowMinimum,"SceneDurationRangeGate",p,sceneNumber,completeSceneSentences,decision,read,usages,optionalUsages,$"Scene {p.Variant}/{p.SceneId} duration {read} is below minimum {decision.MinimumDurationSeconds}."));
        if(read>decision.MaximumDurationSeconds)return(null,TimingFail(NarrationDraftReasonCodes.SceneDurationAboveMaximum,"SceneDurationRangeGate",p,sceneNumber,completeSceneSentences,decision,read,usages,optionalUsages,$"Scene {p.Variant}/{p.SceneId} duration {read} is above maximum {decision.MaximumDurationSeconds}."));
        var draft=new NarrationDraftScene("",p.PlanningId,p.SceneId,p.Variant,p.StoryFrameId,p.PacketId,p.DeterministicChecksum,p.ViewerQuestion,p.LearningObjective,opening,completeSceneSentences,
            closing,incoming,outgoing,usages,optionalUsages,p.DeferredClaims,usages.Concat(optionalUsages).SelectMany(x=>x.QualificationIds).Distinct().ToArray(),p.SafetyRequirements,p.EditorialConstraints,words,completeSceneSentences.Length,read,p.MinimumDuration,p.ExpectedDuration,p.MaximumDuration,"");
    ...
    private static NarrationDraftAuthorityBuildResult TimingFail(string code,string gate,NarrationPlanningScene p,int sceneNumber,IReadOnlyList<NarrationDraftSentence> completeSceneSentences,NarrationDraftSceneTimingDecision d,decimal read,IReadOnlyList<NarrationDraftClaimUsage> required,IReadOnlyList<NarrationDraftClaimUsage> optional,string error)
    {
        var summary=new NarrationDraftSceneFailureSummary(p.Variant,sceneNumber,p.SceneId,p.StoryFrameId,p.PlanningId,p.NarrativeGoal.SectionKey,completeSceneSentences.Count,d.MinimumSentenceCount,d.MaximumSentenceCount,read,d.MinimumDurationSeconds,d.MaximumDurationSeconds,p.RequiredClaims,required.Select(x=>x.ClaimId).ToArray(),optional.Select(x=>x.ClaimId).ToArray(),completeSceneSentences.Where(x=>x.IsTransition).Select(x=>x.SentenceId).ToArray(),[gate],[code]);
