    ...
    IPhase7SceneReferenceCompatibilityPolicy referenceCompatibilityPolicy) : IPhase7ScenePacketInputAuthorityEvaluator
{
        ...
        Phase7ScenePacketInputAuthorityRequest request, CancellationToken token = default)
    {
        ...
        var requirements = projections.ToDictionary(
            x => x.frame.FrameId,
            x => (IReadOnlyList<Phase7SceneReferenceRequirement>)x.result.Requirements
                .Select(r => r with { IsRequired = HasRequiredBinding(knowledge.Authority, r.ReferenceId) })
                .ToArray(),
