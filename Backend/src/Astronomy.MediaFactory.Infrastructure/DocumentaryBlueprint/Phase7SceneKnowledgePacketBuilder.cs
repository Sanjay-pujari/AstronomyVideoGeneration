...
public sealed class Phase7SceneKnowledgePacketBuilder : IPhase7SceneKnowledgePacketBuilder
{
    ...
    private SceneKnowledgePacket BuildOne(Phase7ScenePacketInputAuthority input, StoryFrameAuthorityFrame frame, string variant)
    {
        ...
        var section = source.NarrativeStage;
        if (string.IsNullOrWhiteSpace(section) || string.IsNullOrWhiteSpace(source.SceneRole))
            throw new InvalidOperationException($"Source scene '{source.SceneId}' has no authoritative section/role.");
        var requirements = input.ReferenceRequirements.TryGetValue(frame.FrameId, out var governed) ? governed :
            frame.KnowledgeReferenceIds.Select((id, i) => new Phase7SceneReferenceRequirement(id, variant, i == 0, true,
                "Phase6StoryFramesAuthority", $"frames/{frame.FrameId}/knowledgeReferenceIds/{i}")).ToArray();
        ...
        var exactOptional = resolved.Where(x => !x.Requirement.IsRequired && x.Resolution.Status == Phase7KnowledgeReferenceStatus.Resolved)
            .SelectMany(x => x.Resolution.Claims).DistinctBy(x => x.ClaimId, StringComparer.Ordinal);
        var allowedDomains = DomainTerms(section, source.SceneRole);
        ...
        var optional = exactOptional.Concat(authorityClaims.Where(c => c.Disposition == Phase7ClaimDisposition.Optional && Relevant(c)))
            .Where(c => c.Disposition == Phase7ClaimDisposition.Optional && !c.RequiresHumanReview)
            .DistinctBy(c => c.ClaimId, StringComparer.Ordinal).OrderBy(c => c.ClaimId, StringComparer.Ordinal).ToArray();
        ...
        if (!requirements.Any(x => x.IsPrimary && x.IsRequired)) blocking.Add("P7REF_PRIMARY_REQUIRED_MISSING");
        if (required.Length == 0) blocking.Add($"No relevant Required claim resolves authoritative section '{section}'.");
        ...
        var draft = new SceneKnowledgePacket(packetId,input.ExecutionId,input.PlanId,input.EventId,input.EventFamily,input.Language,
            ...
            required.Any(x=>x.IsLocationDependent),required.Any(x=>x.IsDateTimeDependent),required.Where(x=>x.IsApproximate).Select(x=>$"Claim {x.ClaimId} is approximate.").ToArray(),
    ...
    private static HashSet<string> DomainTerms(string section,string role) => Normalize($"{section}-{role}").Split('-',StringSplitOptions.RemoveEmptyEntries)
        .SelectMany(x=>x switch { "hook" or "opening"=>["identity","recognition","interestingfacts"], "science"=>["scientificstructure","physicalcharacteristics","formation","evolution"], "observation"=>["observation","visibility","timing","locationdependence"], "culture"=>["cultureandmythology","regionaltraditions"], _=>new[]{x} }).ToHashSet(StringComparer.Ordinal);
