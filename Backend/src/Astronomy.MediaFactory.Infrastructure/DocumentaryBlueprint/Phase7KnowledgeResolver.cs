...
public sealed class Phase7KnowledgeResolver : IPhase7KnowledgeResolver
{
    ...
    public Phase7KnowledgeResolver() : this(new Phase7KnowledgeSectionAdapterRegistry()) { }
    public Phase7KnowledgeResolver(Phase7KnowledgeSectionAdapterRegistry registry) => this.registry = registry;
    ...
    public ResolvedNarrationKnowledge Resolve(CertifiedKnowledgePayload payload, FamilyNarrationProfile profile)
    {
        ...
        Read(payload.EvergreenJson, Phase7KnowledgeOrigin.Evergreen, payload.EvergreenPayloadId ?? payload.PayloadId, payload, candidates, warnings, issues);
        Read(payload.RawDataJson, Phase7KnowledgeOrigin.Event, payload.EventId, payload, candidates, warnings, issues);
        ...
        foreach (var candidate in candidates.OrderBy(x=>x.SemanticIdentity,StringComparer.Ordinal))
        {
            ...
            if (Equivalent(prior.Text,candidate.Text)) continue;
    ...
    private void Read(string? json,Phase7KnowledgeOrigin origin,string id,CertifiedKnowledgePayload payload,List<Phase7AdapterClaimCandidate> output,List<string> warnings,List<string> issues)
    {
        ...
        try
        {
            ...
            foreach(var section in doc.RootElement.EnumerateObject())
            {
                ...
                if(adapter is null) { warnings.Add($"P7KNOWLEDGE_UNKNOWN_SECTION:{origin}:{section.Name}"); continue; }
                ...
                var extracted=adapter.Extract(context); output.AddRange(extracted.Claims); warnings.AddRange(extracted.Warnings); issues.AddRange(extracted.BlockingIssues);
            }
        }
    ...
    private static CertifiedNarrationClaim Claim(Phase7AdapterClaimCandidate c,CertifiedKnowledgePayload p,List<string> issues)
    {
        ...
        var exactField=certified.Where(x=>c.SourceIds.Contains(x.SourceId,StringComparer.OrdinalIgnoreCase)).ToArray();
    ...
    private static bool Equivalent(string a,string b)=>string.Equals(a.Trim(),b.Trim(),StringComparison.OrdinalIgnoreCase);
