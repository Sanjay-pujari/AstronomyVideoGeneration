...
public sealed class NarrationGeneratorV5(ILogger<NarrationGeneratorV5> logger, IRequiredSemanticFactResolver requiredSemanticFactResolver, INarrationRealizer narrationRealizer, IAstronomyFamilyProfileResolver familyProfileResolver, NarrationPromptComposer? promptComposer = null, DocumentaryStyleDirector? styleDirector = null, INarrationPerformer? narrationPerformer = null)
{
    ...
    public async Task<NarrationGeneratorV5Result> BuildAndWriteDiagnosticsAsync(BatchGenerateFromPlansRequest request, BatchGenerateFromPlansResponse response, CancellationToken cancellationToken)
    {
        ...
        var projectionScenes = allFactCards.Select(card => new {
            variant = card.Format, sceneNumber = card.SceneOrder, card.BlueprintSceneId, StoryFrameId = card.SourceStoryFrameId,
            blueprintKnowledgeReferenceCount = card.SelectedKnowledgeReferenceIds?.Count ?? 0, storyFrameKnowledgeReferenceCount = 0,
            selectedClaimCount = card.SelectedClaimIds?.Count ?? 0,
            requiredFactCount = card.KnowledgeFacts?.Count(f => f.Required) ?? 0,
            optionalFactCount = card.KnowledgeFacts?.Count(f => !f.Required) ?? 0,
            resolvedFactCount = card.KnowledgeFacts?.Count ?? 0, knowledgeFactCount = card.Facts.Count,
        ...
        await WriteAllTextUtf8Async(sceneFactProjectionDiagnosticsPath, JsonSerializer.Serialize(new {
            scenes = projectionScenes,
            variants = new { longExpectedSceneCount = longStoryFrames.Frames.Count, longCardsProduced = longSceneFactCards.Cards.Count, longScenesWithZeroFacts = longSceneFactCards.Cards.Count(c => c.Facts.Count == 0), shortExpectedSceneCount = shortStoryFrames.Frames.Count, shortCardsProduced = shortSceneFactCards.Cards.Count, shortScenesWithZeroFacts = shortSceneFactCards.Cards.Count(c => c.Facts.Count == 0), placeholderFactCount, factsProjectedFromCertifiedKnowledge = allFactCards.Sum(c => c.Facts.Count), providerInvocationPreventedDueToFactProjectionFailure = placeholderFactCount > 0 }
        }, JsonOptions), cancellationToken);
        if (placeholderFactCount > 0) throw new InvalidOperationException("P7_SCENE_FACT_PLACEHOLDER_DETECTED: certified fact projection contained editorial placeholder text.");
        ...
        var narrationInputNormalization = NarrationInputNormalizer.Normalize(
            ...
            languageProfile);
        var projectionDiagnostics = NarrationContextProjectionValidator.Validate(
            requestedFormats, longSceneFactCards, shortSceneFactCards, longRawNarrative, shortRawNarrative,
            narrationInputNormalization.Context);
        await WriteAllTextUtf8Async(narrationInputNormalizationDiagnosticsPath,
            JsonSerializer.Serialize(new { normalization = narrationInputNormalization.Diagnostics, contextProjection = projectionDiagnostics }, JsonOptions), cancellationToken);
        ...
        try
        {
            ...
            foreach (var format in requestedFormats)
            {
                ...
                var repairGuidance = ReadRepairGuidance(outputRoot, format).ToList();
                IReadOnlyDictionary<int, string>? parsed = null;
                NarrationProviderCallResult? completedCall = null;
                string requestChecksum = string.Empty;
                ...
                for (var attempt = 1; attempt <= 2 && parsed is null; attempt++)
                {
                    ...
                    await WriteAllTextUtf8Async(formatRequestPath, JsonSerializer.Serialize(new
                    {
                        ...
                        modelOrDeployment = narrationPerformer.ModelOrDeployment, variant = format, attempt,
                        expectedSceneCount = contexts.Count, systemPrompt = performerPrompt, userPrompt = actualUserPrompt,
                        actualProviderPromptChecksum = requestChecksum, diagnosticPromptChecksum = requestChecksum, repairGuidance,
                        sceneInputs = contexts.Select((c, i) => new { sceneNumber = i + 1,
                            requiredFactCount = c.VerifiedFacts.Count, optionalFactCount = 0,
                            resolvedFactCount = c.VerifiedFacts.Count,
                            meaningfulSubjectCount = HasMeaningfulSubject(c.AudienceOutcome) ? 1 : 0,
                            performerPromptFactCount = c.VerifiedFacts.Count })
                    }, JsonOptions), cancellationToken);
                    ...
                    finally
                    {
                        ...
                        {
                            providerCallId = call.ProviderCallId, attemptId = call.AttemptId, variant = format,
                ...
                var scenesForFormat = contexts.OrderBy(c => c.SceneOrder).Select((context, index) => new NarrationV5Scene(
                    context.SceneId, context.KnowledgeGoal, parsed[index + 1], context.VerifiedFacts.Select(f => f.FactKey).ToArray(), [], "ProviderResponse")).ToArray();
                ...
                await WriteAllTextUtf8Async(scriptPath, JsonSerializer.Serialize(new { variant = format, diagnosticOnly = true, finalNarrationAuthority = false, scenes = scenesForFormat.Select((s, i) => new { sceneNumber = i + 1, narrationText = s.NarrationText, finalNarrationTextSource = s.FinalNarrationTextSource }) }, JsonOptions), cancellationToken);
                var canonicalMetadataPath = Path.Combine(format.Equals("short", StringComparison.OrdinalIgnoreCase) ? shortRoot : longRoot, "provider-response-metadata.json");
                File.Copy(Path.Combine(Path.GetDirectoryName(canonicalMetadataPath)!, $"provider-response-metadata.attempt-{llmRequestCounts.GetValueOrDefault(format)}.json"), canonicalMetadataPath, true);
                providerDiagnostics.Add(new { variant = format, providerInvocationCount = llmRequestCounts.GetValueOrDefault(format), providerCallId = completedCall!.ProviderCallId,
                    attemptId = completedCall.AttemptId, providerName = completedCall.ProviderName,
                    modelOrDeployment = completedCall.ModelOrDeployment, completedCall.RequestStartedUtc, completedCall.RequestCompletedUtc,
                    providerResponseReceived = true, providerResponseLength = completedCall.ResponseCharacterCount, responseChecksum = completedCall.ResponseChecksum,
                    responseParseSucceeded = true, fallbackUsed = false, fallbackReason = (string?)null, actualProviderPromptChecksum = requestChecksum,
                    diagnosticPromptChecksum = requestChecksum, finalNarrationTextSource = "ProviderResponse",
                    factualInputs = contexts.Select((c, i) => new {
                        sceneNumber = i + 1, requiredFactCount = c.VerifiedFacts.Count, optionalFactCount = 0,
                        resolvedFactCount = c.VerifiedFacts.Count,
                        meaningfulSubjectCount = HasMeaningfulSubject(c.AudienceOutcome) ? 1 : 0,
                        performerPromptFactCount = c.VerifiedFacts.Count }).ToArray() });
            }
        ...
        var wholeDocumentGenerationUsed = requestedFormats.All(format => llmRequestCounts.GetValueOrDefault(format) > 0);
    ...
    private static string BuildPerformerSystemPrompt(LanguageProfile profile)
        => "OUTPUT LANGUAGE\n\n" + profile.OutputInstruction + "\n\n" +
           "Write polished, viewer-facing astronomy documentary narration. Context metadata is private reasoning material, never spoken text. Convert each purpose and governed fact into connected natural prose: explain meaning rather than listing values, keep every passage distinct, and preserve factual grounding. Attribute mythology and cultural traditions, and distinguish astronomy from astrology. Write Short independently rather than copying or compressing Long.\n\nNEVER OUTPUT\nField names; IDs; enum or stage labels; phase, authority, contract, validation, diagnostic, system, developer, or producer language; placeholder transition names; internal instructions; JSON fragments; or Markdown headings inside narration. Never quote source JSON. Scene IDs may exist only in the required response structure and never in narrationText.\n\nReturn exactly one narration block for every supplied canonical scene, in unchanged order, using the existing narration JSON output contract. Do not return an unmapped full-text blob.\n\nOpen with immediate curiosity and close memorably. Use natural, human, calm, cinematic prose.\n\nFINAL OUTPUT CONSTRAINTS\n\n" + profile.OutputInstruction + "\nUse consistent terminology: " + string.Join("; ", profile.Terminology.Select(kv => $"{kv.Key} → {kv.Value}")) + ".";
            ...
            if (File.Exists(compositionPath))
            {
                ...
                if (composition is not null && composition.OrderedScenes.Count > 0)
                {
                    ...
                    return new StoryFrameSource(compositionPath, composition.OrderedScenes
                        ...
                        .Select(scene => new StoryFrameNarrationSource(scene.SceneId, scene.SceneNumber,
                            string.IsNullOrWhiteSpace(scene.StoryFrameId) ? scene.SceneId : scene.StoryFrameId,
                            scene.NarrationBrief + repair,
                            scene.RequiredFacts.Select(f => new SceneKnowledgeFact(f.ClaimId, f.Fact, f.KnowledgeReferenceIds, f.SourceIds,
    ...
    private static string? SanitizePreview(string? value)
    {
        ...
        return sanitized[..Math.Min(160, sanitized.Length)];
    }
    ...
    // Final NarrationV5Scene construction intentionally has no documentary-script or deterministic
    // writer/editor path. Those artifacts are diagnostics/context only; provider narrationText is
    // mapped once above and is never rewritten after parsing.
    ...
    private static ObjectKnowledgeValue? BuildStructuredObjectKnowledge(string familyId, ImmutableArray<AstronomicalObjectValue> objects)
    {
    ...
    private static object BuildFamilyProfileV1CompatibilityRuntimeDiagnostics(object compatibilityDiagnostics, AstronomyFamilyProfile legacyProfile)
    {
        ...
        object[] Rows(string format, ImmutableArray<FamilyNarrativeBeatV1> beats) => beats.Select(b => new
        {
            ...
            runtimeRequiredFactTypes = RequiredProfileTypes(legacyProfile, b.BeatRole, format).ToArray(),
            runtimeOptionalFactTypes = OptionalProfileTypes(legacyProfile, b.BeatRole, format).ToArray()
        }).Cast<object>().ToArray();
    ...
    private static IEnumerable<string> RequiredProfileTypes(AstronomyFamilyProfile profile, string role, string format)
    {
        var beat = FindProfileBeat(profile, role, format);
        if (beat is not null)
        ...
        if (!profile.ContentNature.Contains("Event", StringComparison.OrdinalIgnoreCase))
            return profile.RequiredFactTypes.Where(t => !Regex.IsMatch(t, "Date|Time|Peak|Window", RegexOptions.IgnoreCase)).Distinct(StringComparer.OrdinalIgnoreCase);
        return profile.RequiredFactTypes.Distinct(StringComparer.OrdinalIgnoreCase);
    }

    private static IEnumerable<string> OptionalProfileTypes(AstronomyFamilyProfile profile, string role, string format)
    {
        var beat = FindProfileBeat(profile, role, format);
        if (beat is not null)
            return beat.Requirements.Where(r => !r.BlocksPhase7).Select(r => r.SemanticCapabilityId.Value).Distinct(StringComparer.OrdinalIgnoreCase);
        return profile.OptionalFactTypes.Distinct(StringComparer.OrdinalIgnoreCase);
    }

    private static FamilyNarrativeBeatV1? FindProfileBeat(AstronomyFamilyProfile profile, string role, string format)
    {
        var catalog = new AstronomyFamilyProfileCatalogV1();
        if (!catalog.TryGet(profile.FamilyId, out var v1)) return null;
...
public sealed record NarrationContextBeat(string KnowledgeGoal, string AudienceOutcome, string EditorialIntent, IReadOnlyList<NarrationVerifiedFact> VerifiedFacts, IReadOnlyList<string> ScientificConstraints, string? ObservationObjective, string TransitionGoal, string Tone, string NarrativeRhythm, IReadOnlyList<string> SuccessCriteria, string? OptionalProducerNotes, string SceneId = "", int SceneOrder = 0, string Format = "", int TargetDurationSeconds = 0, string? PreviousSceneContext = null);
    ...
    public static NarrationInputNormalizationResult Normalize(JsonElement? longContract, JsonElement? shortContract, JsonElement? decisionLog, JsonElement? editorialBrief, JsonElement? producerNotes, JsonElement? styleContract, DocumentaryPerformerSceneFactCards factCards, RequiredSemanticFactResolutionResult? resolvedFacts, string voiceProfile, string orchestrationVersion, LanguageProfile languageProfile)
    {
        ...
        foreach (var format in source.Formats)
        {
            ...
            foreach (var beat in format.Beats)
            {
                ...
                var safe = new NarrationSafeContext(format.Format, beat.SceneId, beat.SceneId, ResolveRole(facts, languageProfile), beat.KnowledgeGoal, beat.AudienceOutcome, beat.EditorialIntent, beat.ObservationObjective, beat.ObservationObjective, beat.TransitionGoal, beat.Tone, beat.NarrativeRhythm, beat.TargetDurationSeconds > 0 ? Math.Max(12, beat.TargetDurationSeconds * 2) : format.Format.Equals("short", StringComparison.OrdinalIgnoreCase) ? 35 : 95, facts, notes, constraints);
                safeContexts.Add(safe);
                handoffDiagnostics.Add(new NarrationSafeContextHandoffDiagnostic(format.Format, beat.SceneId, beat.SceneId, beat.VerifiedFacts.Count, facts.Select(f => f.FactKey).ToArray(), ignoredFacts, JsonSerializer.Serialize(new { scene = beat.SceneId, beat = beat.SceneId, facts = safe.SpeakableFacts.Select(f => new { factType = f.FactKey, value = f.SpeakableValue }) })));
                beats.Add(new NarrationContextBeat(safe.KnowledgeGoal, safe.AudienceOutcome, safe.EditorialIntent, facts.Select(f => new NarrationVerifiedFact(f.FactKey, f.SpeakableValue, f.FactType, f.CanonicalUnit)).ToArray(), safe.Constraints, safe.ObservationObjective, safe.TransitionGoal, safe.Tone, safe.Rhythm, beat.SuccessCriteria.Select(_ => SemanticTemplate("successCriteria", languageProfile)).Distinct().ToArray(), notes.Length == 0 ? null : string.Join(" ", notes.Select(n => n.SemanticInstruction)), beat.SceneId, beat.SceneOrder, format.Format, beat.TargetDurationSeconds, beat.PreviousSceneContext));
            }
...
public static class NarrationContextBuilder
{
    ...
    private static IReadOnlyList<NarrationContextBeat> BuildBeats(string format, JsonElement? contract, JsonElement? styleContract, SceneFactCardSet cards, RequiredSemanticFactResolutionResult? resolvedFacts, string voiceProfile)
    {
        ...
        if (contractBeats.Length == 0)
        {
            return cards.Cards.OrderBy(card => card.SceneOrder).Select((card, index) =>
            {
                var facts = card.Facts.Concat(card.Observations).Concat(card.Science).Concat(card.RequiredMentions)
                    .Where(value => !string.IsNullOrWhiteSpace(value) && !ContainsForbiddenVisualLanguage(value))
                    .Distinct(StringComparer.OrdinalIgnoreCase)
    ...
    private static string NormalizeVariant(string format) => format.Trim().ToLowerInvariant() switch
    {
        "long" => "long", "short" => "short",
        _ => throw new InvalidOperationException($"NarrationContextProjectionUnknownVariant: variant='{format}'.")
    };
    ...
    private static IReadOnlyList<NarrationVerifiedFact> ProjectCardKnowledgeFacts(SceneFactCard card)
    {
        var candidates = card.KnowledgeFacts ?? [];
        var placeholders = candidates.Where(f => SceneFactCardGenerator.IsPlaceholderFact(f.Statement)).Select(f => f.Statement).ToArray();
        if (placeholders.Length > 0)
            throw new InvalidOperationException($"P7_SCENE_FACT_PLACEHOLDER_DETECTED: scene '{card.SceneId}' contains non-factual projection candidates: {string.Join("; ", placeholders)}");
        return candidates.Where(f => !string.IsNullOrWhiteSpace(f.Statement) && f.KnowledgeReferenceIds.Count > 0 && f.SourceIds.Count > 0)
            .Select(f => new NarrationVerifiedFact(f.ClaimId, Clean(f.Statement), "CertifiedKnowledgeClaim"))
            .DistinctBy(f => f.Value, StringComparer.OrdinalIgnoreCase).ToArray();
    }
...
public static class SceneFactCardGenerator
{
    public static SceneFactCardSet Build(string format, ProducerNotesContract notes, string orchestrationVersion,
        IReadOnlyList<StoryFrameNarrationSource> storyFrames, RequiredSemanticFactResolutionResult resolution)
    {
        var enriched = storyFrames.Select(frame =>
        {
            var beat = resolution.Beats.FirstOrDefault(b => b.Format.Equals(format, StringComparison.OrdinalIgnoreCase) &&
                (b.SceneId.Equals(frame.SceneId, StringComparison.OrdinalIgnoreCase) || b.DocumentaryBeatId.Equals(frame.SceneId, StringComparison.OrdinalIgnoreCase)));
            if (beat is null) return frame;
            var resolved = beat.RequiredFacts.Concat(beat.OptionalFacts).Where(f => f.SafeForNarration).Select(f =>
                new SceneKnowledgeFact(f.FactKey, SemanticFactValueRealizer.Instance.Realize(f, LanguageProfileResolver.Resolve(f.Language)).SpeakableValue ?? string.Empty,
                    f.SourceInputs?.Where(v => !string.IsNullOrWhiteSpace(v)).ToArray() ?? [f.SourceField], [f.SourceArtifact], f.Confidence,
                    string.IsNullOrWhiteSpace(f.DerivationRuleId) ? [] : [$"Derived under {f.DerivationRuleId}."], f.Requiredness.Equals("Required", StringComparison.OrdinalIgnoreCase)));
            return frame with { KnowledgeFacts = (frame.KnowledgeFacts ?? []).Concat(resolved).DistinctBy(f => f.ClaimId, StringComparer.Ordinal).ToArray() };
        }).ToArray();
        return Build(format, notes, orchestrationVersion, enriched);
    }
    ...
    private static readonly Regex PlaceholderFactRegex = new(@"(?:\bviewer\s+goal\b|\btakeaway\b|\badvance\d+\b|orion\s+gold\s+scene|transition\s+intent|purpose:|scene\s+outcome|final\s+narration\s+remains\s+owned|observation\s+remains\s+grounded\s+in\s+the\s+confirmed\s+story\s+details)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    public static bool IsPlaceholderFact(string? value) => !string.IsNullOrWhiteSpace(value) && PlaceholderFactRegex.IsMatch(value);
...
{
    public static RawNarrative Build(string format, ProducerNotesContract notes, string orchestrationVersion, IReadOnlyList<StoryFrameNarrationSource>? canonicalFrames = null)
    {
        var normalizedFormat = format.Trim().ToLowerInvariant() switch
        {
            "long" => "long", "short" => "short",
            _ => throw new InvalidOperationException($"RawNarrativeUnknownVariant: variant='{format}'.")
        };
        if (canonicalFrames is { Count: > 0 })
        {
            var noteLookup = notes.Briefs.Where(b => b.FormatRequirement.Equals(normalizedFormat, StringComparison.OrdinalIgnoreCase))
                .GroupBy(b => b.SceneId, StringComparer.OrdinalIgnoreCase).ToDictionary(g => g.Key, g => g.First(), StringComparer.OrdinalIgnoreCase);
            var canonicalScenes = canonicalFrames.OrderBy(f => f.SceneOrder).Select((frame, index) =>
            {
                noteLookup.TryGetValue(frame.SceneId, out var note);
                return new RawNarrativeScene(frame.SceneId, frame.SceneOrder, ResolveRole(note?.NarrativeGoal ?? frame.NarrationMapping, index),
                    note?.KeyFacts.Select(f => $"{f.Name}: {f.Value}").ToArray() ?? [],
                    SplitSentences(note?.SceneStory ?? frame.NarrationMapping), SplitSentences(note?.ObservationGuidance ?? string.Empty),
                    ["Do not invent unconfirmed event details."], note?.TransitionContext ?? frame.NarrationMapping,
                    normalizedFormat == "short" ? 12 : 28, frame.SceneId, frame.FrameId);
            }).ToArray();
            return new RawNarrative("AstroPulse-RawNarrative-v1", orchestrationVersion, normalizedFormat, notes.Language, canonicalScenes);
        }
        var selected = notes.Briefs.Where(b => b.FormatRequirement.Equals(normalizedFormat, StringComparison.OrdinalIgnoreCase)).OrderBy(b => b.SceneOrder).ToArray();
        ...
        return new RawNarrative("AstroPulse-RawNarrative-v1", orchestrationVersion, normalizedFormat, notes.Language, scenes);
    }
...
public sealed record NarrationContextProjectionDiagnostics(
    int LongCompositionSceneCount, int ShortCompositionSceneCount,
    int LongRawNarrativeSceneCount, int ShortRawNarrativeSceneCount,
    int LongSceneFactCardCount, int ShortSceneFactCardCount,
    int LongSafeContextBeatCount, int ShortSafeContextBeatCount,
    bool LongSceneIdsMatch, bool ShortSceneIdsMatch, bool VariantLeakageDetected,
    bool LlmInvocationPreventedDueToEmptyContext, string ContextProjectionSource,
    string? FirstLongMismatch, string? FirstShortMismatch);

public static class NarrationContextProjectionValidator
{
    public static NarrationContextProjectionDiagnostics Validate(IReadOnlyList<string> requestedFormats,
        SceneFactCardSet longCards, SceneFactCardSet shortCards, RawNarrative longRaw, RawNarrative shortRaw,
        NarrationContextDocument context)
    {
        var requested = requestedFormats.Select(Normalize).ToHashSet(StringComparer.OrdinalIgnoreCase);
        var longExpected = longCards.Cards.OrderBy(c => c.SceneOrder).Select(c => c.SceneId).ToArray();
        var shortExpected = shortCards.Cards.OrderBy(c => c.SceneOrder).Select(c => c.SceneId).ToArray();
        var longActual = Beats("long").Select(b => b.SceneId).ToArray();
        var shortActual = Beats("short").Select(b => b.SceneId).ToArray();
        var firstLong = FirstMismatch(longExpected, longActual);
        var firstShort = FirstMismatch(shortExpected, shortActual);
        var longIdSet = longExpected.ToHashSet(StringComparer.OrdinalIgnoreCase);
        var shortIdSet = shortExpected.ToHashSet(StringComparer.OrdinalIgnoreCase);
        var leakage = longActual.Any(shortIdSet.Contains) || shortActual.Any(longIdSet.Contains)
            || longCards.Cards.Any(c => Normalize(c.Format) != "long") || shortCards.Cards.Any(c => Normalize(c.Format) != "short");
        var empty = (requested.Contains("long") && longExpected.Length == 0) || (requested.Contains("short") && shortExpected.Length == 0)
            || (requested.Contains("long") && longActual.Length == 0) || (requested.Contains("short") && shortActual.Length == 0);
        var invalidIdentity = longActual.Any(string.IsNullOrWhiteSpace) || shortActual.Any(string.IsNullOrWhiteSpace)
            || longActual.Distinct(StringComparer.OrdinalIgnoreCase).Count() != longActual.Length
            || shortActual.Distinct(StringComparer.OrdinalIgnoreCase).Count() != shortActual.Length;
        var meaningless = context.Formats.SelectMany(f => f.Beats).Any(b => string.IsNullOrWhiteSpace(b.KnowledgeGoal)
            && b.VerifiedFacts.Count == 0 && string.IsNullOrWhiteSpace(b.ObservationObjective) && string.IsNullOrWhiteSpace(b.OptionalProducerNotes));
        var diagnostics = new NarrationContextProjectionDiagnostics(longExpected.Length, shortExpected.Length,
            longRaw.Scenes.Count, shortRaw.Scenes.Count, longCards.Cards.Count, shortCards.Cards.Count,
            longActual.Length, shortActual.Length, firstLong is null, firstShort is null, leakage, empty,
            "CanonicalNarrativeCompositionRequest/SceneFactCards", firstLong, firstShort);
        if (empty || invalidIdentity || meaningless || leakage || firstLong is not null || firstShort is not null
            || longRaw.Scenes.Count != longExpected.Length || shortRaw.Scenes.Count != shortExpected.Length)
            throw new InvalidOperationException($"NarrationContextProjectionInvariantFailed: expectedLong={longExpected.Length}; actualLong={longActual.Length}; expectedShort={shortExpected.Length}; actualShort={shortActual.Length}; firstLongMismatch={firstLong ?? "none"}; firstShortMismatch={firstShort ?? "none"}; variantLeakage={leakage}; emptyRequestedContext={empty}; invalidIdentity={invalidIdentity}; meaninglessBeat={meaningless}. LLM invocation prevented.");
        return diagnostics;

        IReadOnlyList<NarrationContextBeat> Beats(string variant) => context.Formats.FirstOrDefault(f => Normalize(f.Format) == variant)?.Beats ?? [];
    }

    private static string Normalize(string? variant) => variant?.Trim().ToLowerInvariant() switch
    {
        "long" => "long", "short" => "short",
        _ => throw new InvalidOperationException($"NarrationContextProjectionUnknownVariant: variant='{variant ?? "<missing>"}'.")
    };

    private static string? FirstMismatch(IReadOnlyList<string> expected, IReadOnlyList<string> actual)
    {
        for (var i = 0; i < Math.Max(expected.Count, actual.Count); i++)
            if (i >= expected.Count || i >= actual.Count || !expected[i].Equals(actual[i], StringComparison.OrdinalIgnoreCase))
                return $"index={i}; expected={(i < expected.Count ? expected[i] : "<none>")}; actual={(i < actual.Count ? actual[i] : "<none>")}";
        return null;
    }
}
...
{
    private static readonly string[] Registry = ["PlanetPairing", "PlanetGrouping", "SolarEclipse", "LunarEclipse", "Eclipse", "Occultation", "MeteorShower", "NamedFullMoon", "FullMoon", "Constellation", "PlanetProfile", "Comet", "DeepSkyObject", "Nebula", "Galaxy", "BlackHoleOrScientificExplainer"];
    private static readonly string[] DomainFacts = ["ApparentAlignmentExplanation", "ApparentPairingScience", "PhysicalProximityClarification", "PerspectiveExplanation", "WhyPlanetsAppearClose", "ScientificImportance", "ScientificMechanism", "Mechanism", "DomainScientificKnowledge", "MoonAppearance", "MoonPhaseKnowledge", "SeasonalName", "ApparentGrouping"];
    ...
    private static object KnowledgeFor(string family) => family.ToLowerInvariant() switch
    {
        ...
        "namedfullmoon" or "fullmoon" => new { mechanism = "a full Moon occurs when the Moon is opposite the Sun in the sky and its near side is broadly illuminated", scientificSignificance = "named full moons connect stable lunar phase geometry with seasonal skywatching traditions", observingPrinciples = "moonrise timing and local horizon conditions must be verified before giving local advice", constraints = new[] { "Do not invent folklore origins.", "Do not guarantee horizon visibility." } },
        "solareclipse" => new { mechanism = "a solar eclipse occurs when the Moon passes between Earth and the Sun and its shadow falls on Earth", scientificSignificance = "eclipses reveal precise Sun-Earth-Moon geometry and, during totality, the solar corona", observingPrinciples = "direct solar viewing requires certified solar protection except during the brief verified totality interval", constraints = new[] { "Never imply unsafe solar viewing is acceptable.", "Do not claim worldwide visibility." } },
        "lunareclipse" => new { mechanism = "a lunar eclipse occurs when the Moon passes through Earth's shadow", scientificSignificance = "the Moon can look coppery or red because Earth's atmosphere bends and filters sunlight into the shadow", observingPrinciples = "visibility depends on whether the Moon is above the horizon during the eclipse", constraints = new[] { "Do not add solar-viewing safety requirements.", "Do not claim worldwide visibility." } },
        "planetgrouping" => new { mechanism = "multiple planets can appear grouped because Earth sees them projected onto nearby directions in the sky", scientificSignificance = "groupings make orbital motion and ecliptic geometry easy to compare", observingPrinciples = "identify each member from verified object data and describe apparent grouping only", constraints = new[] { "Do not imply physical clustering.", "Do not infer members from a title." } },
        "constellation" => new { mechanism = "a constellation is an officially recognized sky region, not a physically bound star cluster", scientificSignificance = "constellations organize sky navigation, mythology, and object finding", observingPrinciples = "use verified pattern, region, season, or major-star knowledge when available", constraints = new[] { "Do not force event timing.", "Do not treat mythology as science." } },
        "deepskyobject" => new { mechanism = "deep-sky objects are galaxies, nebulae, or clusters beyond the Solar System", scientificSignificance = "they reveal stellar birth, stellar death, galactic structure, and cosmic distance scales", observingPrinciples = "dark skies, optical aid, and object type shape observation advice", constraints = new[] { "Do not force event timing.", "Do not invent discovery or distance values." } },
        _ => new { mechanism = "apparent line-of-sight geometry", apparentCloseness = true, viewpoint = "Earth", physicalProximity = false, scientificSignificance = "apparent alignments show how orbital motion changes the sky from Earth's viewpoint", constraints = new[] { "Do not imply physical close approach.", "Do not imply collision risk." } }
    };
    ...
    private static string ToKebab(string value) => Regex.Replace(value, "(?<!^)([A-Z])", "-$1").Trim('-').ToLowerInvariant();
}
...
public sealed class RequiredSemanticFactResolver : IRequiredSemanticFactResolver
{
    ...
    private object BuildObjectKnowledgeTrace(RequirementOccurrence o, SemanticResolutionResultV1 result, RequiredSemanticFactResolutionInput input, IReadOnlyList<ResolvedBeatFacts> beats)
    {
        ...
        return new
        {
            ...
            objectKnowledgeValuePresent = ok is not null, objectKnowledgeSubject = ok?.ObjectIdentity, objectKnowledgeFactCount = ok?.Facts.Length ?? 0, objectKnowledgeFactKeys = ok?.Facts.Select(f => f.Field).ToArray() ?? [],
...
public static class AstronomyFamilyProfileCatalog
{
    ...
    private static readonly IReadOnlyDictionary<string, AstronomyFamilyProfile> Profiles = new Dictionary<string, AstronomyFamilyProfile>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["SolarEclipse"] = new("SolarEclipse", "TimedObservationEvent", "TimedMechanismExplainer", "SkyWatchShort", ["EventIdentity", "EventDateOrWindow", "EclipseCircumstances", "VisibilityRegion", "SafetyGuidance", "DomainScientificKnowledge"], ["StartTime", "PeakTime", "EndTime", "Magnitude", "ObservationLocation"], ["Hook", "Safety", "Timing", "Orientation", "Observation", "Science", "Closing"], ["Hook", "Safety", "Timing", "Science", "Observation", "Closing"], "Solar eclipse safety guidance is mandatory; location-specific visibility must be verified.", "Window or date required.", ["ShadowGeometry", "OrbitalAlignment"], ["Unsafe solar viewing", "Worldwide visibility"], ["Safety must not be omitted"]),
        ["LunarEclipse"] = new("LunarEclipse", "TimedObservationEvent", "TimedMechanismExplainer", "SkyWatchShort", ["EventIdentity", "EventDateOrWindow", "EclipseCircumstances", "VisibilityRegion", "DomainScientificKnowledge"], ["StartTime", "PeakTime", "EndTime", "Magnitude", "MoonAppearance", "ObservationLocation"], ["Hook", "Timing", "Orientation", "Observation", "Science", "Closing"], ["Hook", "Timing", "Science", "Observation", "Closing"], "Visibility must be region-aware; lunar appearance comes from eclipse circumstances or domain knowledge.", "Window or date required.", ["ShadowGeometry", "OrbitalAlignment", "MoonAppearance"], ["Worldwide visibility"], ["Do not include solar-viewing safety as required lunar guidance"]),
        ...
        ["MeteorShower"] = new("MeteorShower", "TimedObservationEvent", "ObservationGuide", "SkyWatchShort", ["EventIdentity", "EventDateOrWindow", "MeteorActivity", "DomainScientificKnowledge"], ["ObservationDirection", "ObservationConditions", "ObservationEquipment", "Radiant", "Zhr", "DarkSkyGuidance"], ["Hook", "Orientation", "Timing", "Observation", "Science", "Closing"], ["Hook", "Timing", "Observation", "Science", "Closing"], "Dark sky and patience guidance when verified.", "Peak window required.", ["CometDebris", "Radiant"], ["Guaranteed counts"], ["No guaranteed meteors"]),
        ["NamedFullMoon"] = new("NamedFullMoon", "TimedObservationEvent", "ObservationExplainer", "SkyWatchShort", ["EventIdentity", "EventDateOrWindow", "FullMoonObservation", "DomainScientificKnowledge"], ["MoonriseTime", "VisibilityRegion", "CulturalNameContext", "CulturalContext"], ["Hook", "Orientation", "Timing", "Observation", "Science", "Closing"], ["Hook", "Timing", "Science", "Observation", "Closing"], "Moonrise/location guidance only when verified.", "Full Moon date or window is required.", ["LunarPhase", "SunEarthMoonGeometry"], ["Unverified folklore", "Guaranteed horizon visibility"], ["Named Moon identity must come from event type or verified metadata"]),
        ["FullMoon"] = new("FullMoon", "TimedObservationEvent", "ObservationExplainer", "SkyWatchShort", ["EventIdentity", "EventDateOrWindow", "FullMoonObservation", "DomainScientificKnowledge"], ["MoonriseTime", "VisibilityRegion"], ["Hook", "Orientation", "Timing", "Observation", "Science", "Closing"], ["Hook", "Timing", "Science", "Observation", "Closing"], "Moonrise/location guidance only when verified.", "Full Moon date or window is required.", ["LunarPhase", "SunEarthMoonGeometry"], ["Unverified folklore", "Guaranteed horizon visibility"], ["Full Moon identity must come from event type or verified metadata"]),
        ["PlanetGrouping"] = new("PlanetGrouping", "TimedObservationEvent", "ObservationExplainer", "SkyWatchShort", ["EventIdentity", "PrimaryObjects", "EventDateOrWindow", "ObservationDirection", "DomainScientificKnowledge"], ["SecondaryAstronomicalObjects", "AngularRelationship", "ObservationLocation", "ObservationEquipment", "ObservationConditions"], ["Hook", "Orientation", "Timing", "Identification", "Observation", "Science", "Closing"], ["Hook", "Timing", "Orientation", "Science", "Observation", "Closing"], "At least three grouped objects are required; describe apparent grouping rather than physical clustering.", "Event date or window is required.", ["ApparentAlignment"], ["Physical closeness", "Unverified weather"], ["No title inference"]),
        ["Constellation"] = new("Constellation", "EducationalObjectProfile", "ObjectProfile", "ConstellationShort", ["ObjectKnowledge"], ["EventIdentity", "ObservationDirection", "CulturalNameContext", "EditorialContext"], ["Hook", "Orientation", "Science", "Significance", "Closing"], ["Hook", "Orientation", "Science", "Significance", "Closing"], "Identification pattern replaces event viewing direction.", "No event date required.", ["StarPattern", "CelestialCoordinates"], ["Required event date"], ["Do not force event structure"]),
    ...
    private static string? MapToProfileId(string? value)
    {
        ...
        if (Regex.IsMatch(text, "SolarEclipse|solar eclipse", RegexOptions.IgnoreCase)) return "SolarEclipse";
        if (Regex.IsMatch(text, "LunarEclipse|lunar eclipse|blood moon", RegexOptions.IgnoreCase)) return "LunarEclipse";
        if (Regex.IsMatch(text, "Eclipse", RegexOptions.IgnoreCase)) return "Eclipse";
...
public sealed record NarrationV5Scene(string SceneId, string ScenePurpose, string NarrationText, IReadOnlyList<string> RequiredFactsCovered, IReadOnlyList<string> Warnings, string FinalNarrationTextSource = "NonProviderDiagnostic");
