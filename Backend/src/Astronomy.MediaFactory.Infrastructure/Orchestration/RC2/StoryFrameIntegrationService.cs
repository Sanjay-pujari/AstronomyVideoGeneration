...
public sealed class StoryFrameIntegrationService(CreativeStoryboardBuilder builder) : IStoryFrameIntegrationService
{
    ...
    public async Task<StoryFrameIntegrationResult> BuildAsync(StoryFrameIntegrationRequest request, CancellationToken cancellationToken)
    {
        ...
        var frames=await builder.BuildCertifiedFramesAsync(request.EditorialContract, request.RequestedVariants, cancellationToken);
        ...
        var authority=new StoryFramesAuthority($"story-frames-{request.ExecutionId}", request.ExecutionId, request.PlanId,
            ...
            request.EditorialContract.SourcePhase4Checksum, nameof(CreativeStoryboardBuilder), CreativeStoryboardBuilder.AuthorityBuilderVersion,
        ...
        var diagnostics=new StoryFrameDiagnostics(request.ExecutionId,nameof(CreativeStoryboardBuilder),CreativeStoryboardBuilder.AuthorityBuilderVersion,
            ...
            frames.GroupBy(x=>x.Variant).ToDictionary(x=>x.Key,x=>x.Count()),frames.GroupBy(x=>x.SceneId).ToDictionary(x=>x.Key,x=>x.Count()),
