    ...
    public IReadOnlyList<PhaseArtifactDefinition> GetDefinitions(int phaseNumber, FamilyCertificationContext context)
    {
        ...
        return phaseNumber switch
        {
            ...
            2 => Rc2CanonicalArtifactCatalog.Phase2AuthorityArtifacts
                ...
                .ToArray(),
            3 => [Req("phase-03-validation",3,v), Req("viewer-question-bank",3,"03-questions/viewer-question-bank.json"), Req("question-answer-set",3,"03-questions/question-answer-set.json"), Req("learning-objectives",3,"03-questions/learning-objectives.json"), Req("question-plan",3,"03-questions/question-plan.json")],
...
public sealed class Phase3Certifier(IPhaseArtifactRegistry r, ICertificationArtifactVerifier v) : PhaseCertifierBase(r,v)
{
    public override int PhaseNumber => 3;
    protected override string PhaseName => "Viewer Curiosity Framework";

    protected override async Task<IEnumerable<CertificationIssue>> ValidateAsync(FamilyCertificationContext c, CancellationToken t)
    {
        var list = new List<CertificationIssue>();
        var bank = await Read(c, "03-questions/viewer-question-bank.json", t);
        var answers = await Read(c, "03-questions/question-answer-set.json", t);
        var objectives = await Read(c, "03-questions/learning-objectives.json", t);
        var plan = await Read(c, "03-questions/question-plan.json", t);
        if (bank is null || answers is null || objectives is null || plan is null)
        {
            list.Add(Issue("P3.ViewerCuriosityAuthorityMissing", "All four Viewer Curiosity authority artifacts are required."));
            return list;
        }

        var questions = Arr(bank.RootElement, "questions", "Questions", "viewerQuestions").ToArray();
        if (questions.Length == 0) list.Add(Issue("P3.QuestionsMissing", "Viewer question bank must contain questions."));
        var ids = questions.Select(q => Str(q, "id", "questionId", "QuestionId")).Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();
        if (ids.Length != questions.Length) list.Add(Issue("P3.QuestionIdMissing", "Every viewer question requires an id."));
        if (ids.Length != ids.Distinct(StringComparer.OrdinalIgnoreCase).Count()) list.Add(Issue("P3.DuplicateQuestionId", "Duplicate question ids detected."));

        var orders = questions.Select(q =>
        {
            var found = q.TryGetProperty("order", out var value) || q.TryGetProperty("Order", out value);
            return found && value.TryGetInt32(out var order) ? order : 0;
        }).ToArray();
        if (!orders.SequenceEqual(Enumerable.Range(1, questions.Length))) list.Add(Issue("P3.NonCanonicalQuestionOrder", "Question order must be canonical 1..N."));

        var objectiveIds = Arr(objectives.RootElement, "learningObjectives", "objectives", "LearningObjectives")
            .Select(o => Str(o, "id", "learningObjectiveId", "objectiveId")).Where(x => !string.IsNullOrWhiteSpace(x)).ToHashSet(StringComparer.OrdinalIgnoreCase);
        foreach (var question in questions)
        {
            var objectiveId = Str(question, "learningObjectiveId", "objectiveId");
            if (!string.IsNullOrWhiteSpace(objectiveId) && !objectiveIds.Contains(objectiveId))
                list.Add(Issue("P3.UnresolvedLearningObjective", $"Question references missing learning objective '{objectiveId}'."));
        }

        var answerQuestions = Arr(answers.RootElement, "questions", "Questions", "answers").ToArray();
        if (answerQuestions.Length != questions.Length) list.Add(Issue("P3.QuestionAnswerCountMismatch", "Question-answer count does not reconcile with the bank."));
        var declaredCount = new[] { "questionCount", "totalQuestions", "count" }.Select(n => plan.RootElement.TryGetProperty(n, out var p) && p.TryGetInt32(out var value) ? value : (int?)null).FirstOrDefault(x => x.HasValue);
        if (declaredCount.HasValue && declaredCount.Value != questions.Length) list.Add(Issue("P3.QuestionPlanCountMismatch", "Question plan count does not reconcile with the bank."));
        return list;
    }
}
