...
public static class VisualDirectingProfiles
{
    ...
    public static readonly VisualDirectingProfile PortraitDirector = new("PortraitDirector", "close-up cinematic camera framing composed from the first frame for a premium mobile-first YouTube Shorts cover", "mobile premium-poster camera language that preserves perfectly circular planetary geometry through framing, perspective, and camera distance only", "premium mobile-first YouTube Shorts cover hierarchy: title first, beautiful circular Jupiter and Venus second, compact Apple-quality observation card third, clean negative space throughout", "Apple keynote poster / Netflix documentary cover / National Geographic astronomy magazine polish", "the viewer recognizes the Jupiter-Venus event before reading, with both planets physically circular and never oval, stretched, or squeezed", "deep twilight atmosphere and horizon glow make the event feel native to mobile", "protect only three zones: top title, middle planets, bottom compact Apple-quality observation card; no side-panel or landscape logic", ["VISUAL DIRECTING PROFILE: PortraitDirector.", "Compose with close-up cinematic camera framing for a premium mobile-first YouTube Shorts cover.", "Portrait hierarchy: 1 premium title, 2 beautiful circular Jupiter and Venus with perfectly circular planetary geometry, 3 compact Apple-quality observation card with Date, Best Time, Direction, Equipment, and Separation, 4 clean negative space."], UniversalAntiDistortionRules);
...
public sealed class ThumbnailCreativeDirector
{
    public ThumbnailCreativeDirection Direct(ThumbnailPromptContract contract)
    {
        ...
        {
            "9:16" => "Portrait close-up cinematic camera framing: make the planets readable through composition, perspective, and camera distance while preserving perfectly circular planetary geometry.",
...
internal static class CreativeBriefPromptBuilder
{
    ...
    private static IEnumerable<string> BuildSections(ThumbnailPromptContract contract, string aspect, LocalizedPromptTerms terms, ThumbnailFormattedGuideFields fields)
    {
        ...
        {
            "portrait" => $"2160x3840 native 9:16 composition; keep the requested output proportional to this premium mobile-first YouTube Shorts cover design even when rendered at {size}.",
        ...
        var objectRendering = aspect == "portrait"
            ? creativeProfile.ObjectRenderingInstructions.Replace("; Jupiter dominant when it is the primary event object", ", with first-glance identity when it is the primary event object", StringComparison.OrdinalIgnoreCase)
            : creativeProfile.ObjectRenderingInstructions;
        yield return $"Object Rendering: {objectRendering}";
        yield return $"Information to Render: {card}. Use absolute date and time only.";
        yield return $"Typography & UI: {typography}";
        if (aspect == "portrait")
        {
            yield return "Scientific Accuracy: Keep celestial bodies physically circular where applicable, naturally separated, and astronomically plausible; preserve perfectly circular planetary geometry; never stretch, squeeze, or turn circular bodies into ovals.";
            ...
            yield return $"Negative Rules: {creativeProfile.FamilyNegativeRules} No relative-day or urgency phrases; no post-processing overlay instructions; no watermark, clutter, tiny text, random objects, empty poster look, external branding, oval bodies, stretched bodies, squeezed bodies, or tall object language.";
        }
        else
        {
...
public static class ThumbnailCompositionProfiles
{
    ...
    public static readonly CompositionProfile PortraitProfile = new("PortraitProfile", "9:16", ["YouTube Shorts", "Instagram Reels"], ["premium mobile-first YouTube Shorts cover", "Netflix astronomy documentary hierarchy", "National Geographic editorial polish", "Apple Event typography discipline", "premium mobile title", "beautiful circular celestial artwork", "compact Apple-quality observation card", "native mobile composition", "never stretched celestial objects", "avoid AI poster feeling"], ["COMPOSITION PROFILE: PortraitProfile for native 9:16 YouTube Shorts and Instagram Reels covers.", "Use close-up cinematic camera framing: a premium mobile title as the first read, beautiful circular event-only celestial artwork as the emotional center, and a compact Apple-quality observation card as the quiet support element.", "Design from the beginning like a Netflix astronomy documentary cover, National Geographic magazine cover, Apple Event poster, or premium space magazine; this must not feel like a resized landscape, cropped poster, infographic, scientific report, or generic AI-generated poster.", "Preserve perfectly circular planetary geometry and keep every planet, moon, or celestial disk physically correct; never stretch, squash, or turn objects into ovals.", "Increase celestial readability only through camera framing, perspective, composition, negative space, and atmospheric lighting, never through distorted scaling.", "Render only the event objects named in the contract; never invent unrelated planets, moons, labels, duplicate labels, watermarks, decorative filler objects, or empty sky that competes with the hierarchy.", "Use premium dark blue astronomy-documentary atmosphere, gold accents, elegant glow, clean luxury typography, mobile-first readability, generous breathing room, and professional editorial restraint.", "The observation card must be compact, glassy, Apple-quality, and subordinate to the artwork; no side panels, no guide table, no bottom strip, no action prompt, no large card, no observation window."], ["Portrait prompt must be generated natively for 9:16.", "CompositionProfile controls close-up framing, phone readability, premium editorial hierarchy, scientific object integrity, and the cover-first hierarchy."]);
