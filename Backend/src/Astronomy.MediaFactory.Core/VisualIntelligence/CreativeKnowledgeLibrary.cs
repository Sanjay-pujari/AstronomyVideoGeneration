...
public sealed class CreativeKnowledgeLibrary : ICreativeKnowledgeLibrary
{
    ...
    {
        yield return Entry(CreativeKnowledgeFamily.PlanetPairing, "What makes these two planets worth looking for together?", "The conjunction is the hero: make the relationship between the two planets the subject, so the viewer understands this as one observable sky moment rather than two isolated dots.", "Create quiet wonder, recognition, and the feeling of being invited outside at the right time.", ["Use paired placement, natural separation, shared visual center, coordinated lighting, and shared negative space to make the conjunction read instantly.", "Prefer a calm twilight or clean sky field with natural atmosphere; add a subtle horizon only when it helps the viewer imagine observing it.", "Order creative decisions as Relationship > Balance > Wonder > Scale."], "Visual partnership: one planet may be naturally larger or brighter, but neither should become a solo portrait; the apparent close approach is the primary subject.", "Speak visually like premium astrophotography and astronomy documentary: twilight realism, observational atmosphere, restraint, and plausible line-of-sight closeness before spectacle.", ["largest-object-wins staging", "huge primary with tiny secondary planet", "poster-like object isolation", "fake glow connecting planets", "artificial nebulae", "fantasy backgrounds", "overly dramatic color explosions", "crowded poster layout", "prompt syntax or provider instructions"], PairingDomains());
    ...
    private static IReadOnlyDictionary<CreativeKnowledgeDomain, IReadOnlyList<string>> PairingDomains() => new Dictionary<CreativeKnowledgeDomain, IReadOnlyList<string>>
    {
        ...
        [CreativeKnowledgeDomain.CompositionPrinciples] = ["Use balanced pairing, natural separation, shared visual center, coordinated lighting, shared atmosphere, and negative space to communicate pairing."],
        [CreativeKnowledgeDomain.ViewerPsychology] = ["Answer why the viewer should look now and where their eye should land first."],
        [CreativeKnowledgeDomain.DocumentaryLanguage] = ["Prefer twilight, observed sky realism, natural atmosphere, premium astrophotography restraint, and factual viewing context; avoid fantasy-space decoration."],
        [CreativeKnowledgeDomain.VisualHierarchy] = ["Relationship first, then balance, then wonder, then scale; the conjunction itself is the primary subject."],
