...
public sealed class DocumentaryNarrativeAcceptanceDecision
{
    ...
    { ConvergenceId=Guard.Required(convergenceId,nameof(convergenceId)); Guard.Enum(status,nameof(status)); Guard.Enum(primaryReason,nameof(primaryReason)); ArgumentNullException.ThrowIfNull(supportingReasons); if(supportingReasons.Any(x=>!Enum.IsDefined(x))||supportingReasons.Distinct().Count()!=supportingReasons.Count||supportingReasons.Contains(primaryReason))throw new ArgumentException("Supporting reasons must be defined, unique, and exclude the primary reason.",nameof(supportingReasons)); SupportingReasons=new ReadOnlyCollection<DocumentaryNarrativeAcceptanceReason>(supportingReasons.ToArray()); CurrentDraftId=Guard.Required(currentDraftId,nameof(currentDraftId)); CurrentDraftVersion=Guard.Required(currentDraftVersion,nameof(currentDraftVersion)); if(currentFindingCount<0||completedCycleCount<0||unresolvedRevisionItemCount<0)throw new ArgumentOutOfRangeException(nameof(currentFindingCount)); Status=status; PrimaryReason=primaryReason; CurrentFindingCount=currentFindingCount; CompletedCycleCount=completedCycleCount; UnresolvedRevisionItemCount=unresolvedRevisionItemCount; Policy=policy??throw new ArgumentNullException(nameof(policy)); Metadata=metadata??throw new ArgumentNullException(nameof(metadata)); }
