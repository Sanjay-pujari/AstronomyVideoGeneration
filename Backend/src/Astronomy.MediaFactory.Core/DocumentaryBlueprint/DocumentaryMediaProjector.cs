...
public sealed class DocumentaryMediaProjector
{
    public DocumentaryMediaProjectionResult Project(DocumentaryMediaProjectionRequest request)
    {
        ...
        var variants=DocumentaryMediaProjectionInventory.Variants.Select(t=>CreateVariant(id,t,plan,request)).ToArray();
    ...
    private static DocumentaryMediaVariant CreateVariant(string projectId,DocumentaryMediaVariantType type,DocumentarySemanticScenePlan plan,DocumentaryMediaProjectionRequest request)
    {
        ...
        var minimum=format==DocumentaryVideoFormat.Long?p.LongMinimumSceneCount:p.ShortMinimumSceneCount;selected=selected.Take(Math.Max(minimum,selected.Length)).ToArray();
    ...
    private static IReadOnlyList<string> Lines(string text,int max,int count){var words=text.Split(' ',StringSplitOptions.RemoveEmptyEntries);var lines=new List<string>();var current="";foreach(var word in words){var next=current.Length==0?word:$"{current} {word}";if(next.Length>max&&current.Length>0){lines.Add(current);current=word;if(lines.Count==count-1)break;}else current=next;}if(lines.Count<count&&current.Length>0)lines.Add(current.Length<=max?current:current[..max].TrimEnd());return lines.Count==0?[text[..Math.Min(text.Length,max)]]:lines;}
}
