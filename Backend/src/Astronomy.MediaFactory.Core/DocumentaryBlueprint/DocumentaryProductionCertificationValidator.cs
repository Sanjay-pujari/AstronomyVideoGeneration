...
public static class DocumentaryProductionCertificationValidator
{
    ...
    private static bool SceneChainValid(DocumentaryMediaScene scene,DocumentaryMediaVariant variant,DocumentaryMediaVariantExecutionRecord vr,IReadOnlyList<DocumentaryMediaAssetPlan> plans,IReadOnlyList<DocumentaryMediaAssetResult> results)
    {
        ...
        if(sceneVideo is null||variantVideo is null||vr.OutputAssetId!=variantVideo.AssetId||!Verified(sceneVideo.AssetId)||!Verified(variantVideo.AssetId))return false;
        ...
        return required.Length>=3&&required.All(Verified)&&required.All(id=>sceneVideo.Dependencies.Any(d=>d.SourceAssetId==sceneVideo.AssetId&&d.TargetAssetId==id)||sceneVideo.Dependencies.Any(d=>d.TargetAssetId==sceneVideo.AssetId&&d.SourceAssetId==id))&&(variantVideo.Dependencies.Any(d=>d.SourceAssetId==variantVideo.AssetId&&d.TargetAssetId==sceneVideo.AssetId)||variantVideo.Dependencies.Any(d=>d.TargetAssetId==variantVideo.AssetId&&d.SourceAssetId==sceneVideo.AssetId));
        bool Verified(string id)=>results.Any(x=>x.AssetId==id&&x.Status==DocumentaryMediaAssetStatus.Verified);
    }
