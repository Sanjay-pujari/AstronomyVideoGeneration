...
public sealed record AstronomyEvent : IEquatable<AstronomyEvent>
{
    ...
    private static IReadOnlyList<AstronomyEventGeometryQuantity> CopyGeometry(IEnumerable<AstronomyEventGeometryQuantity> items) { ArgumentNullException.ThrowIfNull(items); var a=items.Select(x=>x??throw new ArgumentException("Event geometry cannot contain null entries.", nameof(items))).OrderBy(x=>x.QuantityId.Value,StringComparer.Ordinal).ThenBy(x=>x.Epoch?.ToString(),StringComparer.Ordinal).ToArray(); if(a.GroupBy(x=>new{x.QuantityId,Epoch=x.Epoch?.ToString()}).Any(g=>g.Count()>1)) throw new ArgumentException("Event geometry must be unique by quantity ID and epoch.", nameof(items)); return Array.AsReadOnly(a); }
