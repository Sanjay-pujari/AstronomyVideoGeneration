...
public sealed record AstronomyMeasurementUncertainty
{
    public AstronomyMeasurementUncertainty(AstronomyUncertaintyKind kind, decimal lowerValue, decimal upperValue)
    {
        ...
        if ((Kind == AstronomyUncertaintyKind.SymmetricAbsolute || Kind == AstronomyUncertaintyKind.StandardDeviation) && lowerValue != upperValue)
            throw new ArgumentException($"{Kind} uncertainty requires equal lower and upper values.", nameof(upperValue));
    ...
    public static AstronomyMeasurementUncertainty StandardDeviation(decimal value) => new(AstronomyUncertaintyKind.StandardDeviation, value, value);
}
