...
public sealed record AstronomyMeasurementRange
{
    public AstronomyMeasurementRange(AstronomyMeasurement minimum, AstronomyMeasurement maximum)
    {
        ...
        {
            throw new ArgumentException("Measurement range units must match exactly.", nameof(maximum));
        }

        if (Minimum.Unit.Dimension != Maximum.Unit.Dimension)
        {
            throw new ArgumentException("Measurement range dimensions must match.", nameof(maximum));
        }

        if (Minimum.Value > Maximum.Value)
        {
            throw new ArgumentException("Measurement range minimum cannot be greater than maximum.", nameof(minimum));
        }
    }
