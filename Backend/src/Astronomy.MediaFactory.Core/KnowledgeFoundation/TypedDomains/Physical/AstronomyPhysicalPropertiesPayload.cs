...
public sealed record AstronomyPhysicalPropertiesPayload : ITypedAstronomyKnowledgePayload, IEquatable<AstronomyPhysicalPropertiesPayload>
{
        ...
        IEnumerable<AstronomyPhysicalProperty> properties)
    {
        if (!typeId.IsValid)
        {
            throw new ArgumentException("Physical properties payload type ID is required.", nameof(typeId));
        }
    ...
    public bool Equals(AstronomyPhysicalPropertiesPayload? other)
    {
    ...
    private static IReadOnlyList<AstronomyPhysicalProperty> CopyProperties(
        IEnumerable<AstronomyPhysicalProperty> properties)
    {
        ArgumentNullException.ThrowIfNull(properties);

        var ordered = properties
            .Select(property => property ?? throw new ArgumentException(
                "Physical properties cannot contain null entries.",
                nameof(properties)))
            .OrderBy(property => property.Category)
            .ThenBy(property => property.PropertyId.Value, StringComparer.Ordinal)
            .ThenBy(property => property.Qualifier)
            .ToArray();

        if (ordered.Length == 0)
        {
            throw new ArgumentException("At least one physical property is required.", nameof(properties));
        }

        if (ordered.GroupBy(property => new { property.PropertyId, property.Qualifier }).Any(group => group.Count() > 1))
        {
            throw new ArgumentException("Physical properties must be unique by property ID and qualifier.", nameof(properties));
        }
