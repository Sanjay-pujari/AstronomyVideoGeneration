...
public sealed class AstronomyKnowledgeValidationResult
{
    ...
    public AstronomyKnowledgeValidationResult(IEnumerable<AstronomyKnowledgeValidationIssue> issues)
        : this(issues, preserveIssueOrder: false)
    {
    }

    public AstronomyKnowledgeValidationResult(IEnumerable<AstronomyKnowledgeValidationIssue> issues, bool preserveIssueOrder)
    {
        ArgumentNullException.ThrowIfNull(issues);
        var distinctIssues = issues.Select(i => i ?? throw new ArgumentException("Validation issues cannot contain null entries.", nameof(issues)))
            .Distinct();
        var ordered = preserveIssueOrder
            ? distinctIssues.ToArray()
            : distinctIssues
