...
public sealed class AstronomyKnowledgeStatementQueryResult
{
    public AstronomyKnowledgeStatementQueryResult(AstronomyKnowledgeQueryExecutionStatus status, IEnumerable<IAstronomyKnowledgeStatement> items, AstronomyKnowledgeQueryExecutionMetadata metadata, IEnumerable<AstronomyKnowledgeQueryValidationIssue>? validationIssues = null)
    {
        if (!Enum.IsDefined(status)) throw new ArgumentOutOfRangeException(nameof(status));
        ArgumentNullException.ThrowIfNull(items); Metadata = metadata ?? throw new ArgumentNullException(nameof(metadata));
        if (metadata.Target != AstronomyKnowledgeQueryTarget.Statement) throw new ArgumentException("Metadata target must be Statement.", nameof(metadata));
    ...
    public IReadOnlyList<IAstronomyKnowledgeStatement> Items { get; }
