...
public sealed class DocumentaryPhysicalArtifactDescriptorValidator(IDocumentaryContentIdentityFactory identities) : IDocumentaryPhysicalArtifactDescriptorValidator { public IReadOnlyList<string> Validate(DocumentaryPhysicalArtifactDescriptor d) { var e = new List<string>(); if (string.IsNullOrWhiteSpace(d.AssetId)) e.Add("AssetIdRequired"); if (!Path.IsPathFullyQualified(d.PhysicalPath)) e.Add("PhysicalPathMustBeAbsolute"); var segments=d.PhysicalPath.Split(Path.DirectorySeparatorChar,Path.AltDirectorySeparatorChar);if(segments.Select((value,index)=>(value,index)).Any(x=>x.value.Equals("tmp",StringComparison.OrdinalIgnoreCase)&&segments.Take(x.index).Any(y=>y.Equals("attempts",StringComparison.OrdinalIgnoreCase))))e.Add("TemporaryPathRejected"); if (d.Length <= 0) e.Add("LengthMustBePositive"); if (d.AttemptCount <= 0) e.Add("AttemptCountMustBePositive"); if (string.IsNullOrWhiteSpace(d.CorrelationId)) e.Add("CorrelationIdRequired"); if (!identities.IsValid(d.ContentIdentity)) e.Add("ContentIdentityInvalid"); if (d.Checksum.Length != 64 || d.Checksum.Any(x => !Uri.IsHexDigit(x) || char.IsUpper(x))) e.Add("ChecksumInvalid"); if (d.ContentIdentity != $"sha256:{d.Checksum}") e.Add("IdentityChecksumMismatch"); return e.AsReadOnly(); } }
