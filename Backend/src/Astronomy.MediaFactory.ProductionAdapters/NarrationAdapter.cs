...
public sealed class DocumentaryNarrationAudioInspector : IDocumentaryNarrationAudioInspector
{
    ...
    private static int FindData(byte[] b){for(var i=12;i+8<=b.Length;){var size=BinaryPrimitives.ReadInt32LittleEndian(b.AsSpan(i+4,4));if(Encoding.ASCII.GetString(b,i,4)=="data")return Math.Min(size,b.Length-i-8);if(size<0)break;i+=8+size+(size&1);}return 0;}
