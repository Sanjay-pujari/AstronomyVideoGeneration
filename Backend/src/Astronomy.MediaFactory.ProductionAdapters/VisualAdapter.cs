...
public sealed class ExistingDocumentaryVisualProductionAdapter : IDocumentaryProductionVisualAdapter
{
    ...
    { this.options=options.Value;this.router=router;this.fallbackPolicy=fallbackPolicy;this.providers=providers.ToDictionary(x=>x.ProviderId,StringComparer.Ordinal);this.workspaces=workspaces;this.images=images;this.artifacts=artifacts;this.validator=validator;this.registry=registry;this.diagnostics=diagnostics;this.failures=failures; }
    ...
    private static bool IsBelow(string root,string path){var prefix=Path.GetFullPath(root).TrimEnd(Path.DirectorySeparatorChar)+Path.DirectorySeparatorChar;return path.StartsWith(prefix,StringComparison.Ordinal);}
