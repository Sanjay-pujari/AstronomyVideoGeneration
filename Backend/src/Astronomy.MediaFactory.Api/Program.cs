...
app.MapPost("/api/astronomy/v3/narration/preview", (AstronomyV31NarrationPreviewRequest request, ILogger<Program> logger) =>
{
    logger.LogInformation("Astronomy V3.1 narration preview requested for {EventName} in {Language}/{RegionId}.", request.EventName, request.Language, request.RegionId);
...
app.MapPost("/api/visual-lab/compose-overlay", async Task<IResult> (VisualLabComposeOverlayRequest request, IWebHostEnvironment environment, CancellationToken ct) =>
{
    ...
    var renderDebugPath = Path.Combine(outputDirectory, request.UseCelestialAssets ? "overlay-render-debug-v4.json" : "overlay-render-debug.json");
    ...
    var planetAssets = assetLookup.Assets;
    var validation = ValidateVisualLabOverlaySpec(spec, image.Width, image.Height, assetLookup);
    var renderDebug = BuildVisualLabOverlayRenderDebug(spec, assetLookup, validation);
    if (!validation.IsValid)
    {
        foreach (var asset in planetAssets.Values) asset.Image.Dispose();
        await WriteVisualLabDebugJsonAsync(validationPath, validation, ct);
        await WriteVisualLabDebugJsonAsync(renderDebugPath, renderDebug, ct);
        return Results.BadRequest(new { message = "Visual lab overlay validation failed.", validationPath, overlayRenderDebugPath = renderDebugPath, validation });
    }
...
app.MapPost("/api/visual-lab/compose-hero", async Task<IResult> (VisualLabComposeHeroRequest request, IWebHostEnvironment environment, CancellationToken ct) =>
{
    ...
    var outputDirectory = Path.GetDirectoryName(backgroundPath) ?? Directory.GetCurrentDirectory();
    var composedPath = Path.Combine(outputDirectory, "benchmark-hero-conjunction-v2.png");
    var layoutPath = Path.Combine(outputDirectory, "hero-v2-layout.json");
    var validationPath = Path.Combine(outputDirectory, "hero-v2-validation.json");
...
static IReadOnlyList<string> ValidateVisualLabComposeHeroRequest(VisualLabComposeHeroRequest request)
{
    ...
    var supportedFormats = new[] { "Long" };
    if (string.IsNullOrWhiteSpace(request.BackgroundImagePath)) errors.Add("backgroundImagePath is required.");
    if (string.IsNullOrWhiteSpace(request.EventType) || !supportedEvents.Contains(request.EventType, StringComparer.OrdinalIgnoreCase)) errors.Add("eventType must be PlanetConjunction for Visual Lab Hero Benchmark V2.");
    if (string.IsNullOrWhiteSpace(request.Title)) errors.Add("title is required.");
    if (string.IsNullOrWhiteSpace(request.Subtitle)) errors.Add("subtitle is required.");
    if (string.IsNullOrWhiteSpace(request.Format) || !supportedFormats.Contains(request.Format, StringComparer.OrdinalIgnoreCase)) errors.Add("format must be Long for Visual Lab Hero Benchmark V2.");
...
static VisualLabHeroSpec BuildVisualLabHeroSpec(VisualLabComposeHeroRequest request, int width, int height, string composedPath, string layoutPath, string validationPath)
{
    ...
    var titleBand = new VisualLabOverlayRect(safeMargin, safeMargin, width - safeMargin * 2, (int)Math.Round(height * 0.20));
    var heroScene = new VisualLabOverlayRect((int)Math.Round(width * 0.12), (int)Math.Round(height * 0.25), (int)Math.Round(width * 0.76), (int)Math.Round(height * 0.60));
    var planetSprites = new[]
    {
        new VisualLabHeroPlanetPlacement("Jupiter", (int)Math.Round(width * 0.39), (int)Math.Round(height * 0.57), 430, 1, "largest primary focal object, premium NASA-cover gas giant"),
        new VisualLabHeroPlanetPlacement("Venus", (int)Math.Round(width * 0.59), (int)Math.Round(height * 0.47), 300, 2, "second largest luminous documentary focal object"),
        new VisualLabHeroPlanetPlacement("Mercury", (int)Math.Round(width * 0.70), (int)Math.Round(height * 0.60), 190, 3, "smaller tertiary planet, visible without labels")
    };

    return new VisualLabHeroSpec(
        "VisualLabHeroComposerV2",
        ...
        height,
        safeMargin,
        Math.Max(78, (int)Math.Round(width * 0.052)),
        Math.Max(36, (int)Math.Round(width * 0.024)),
        titleBand,
        heroScene,
        planetSprites,
        new[] { "date panel", "tips panel", "direction marker", "equipment panel", "observation details", "callout labels", "icons", "informational UI" },
        "PremiumDocumentaryHeroV2",
...
static VisualLabOverlayValidation ValidateVisualLabOverlaySpec(VisualLabOverlaySpec spec, int width, int height, VisualLabPlanetAssetLookup? assetLookup = null)
{
    ...
    var jupiterRendered = IsVisualLabPlanetAssetRenderable(planetAssets, "jupiter", GetVisualLabCelestialAssetSpriteWidth("Jupiter"));
    var venusRendered = IsVisualLabPlanetAssetRenderable(planetAssets, "venus", GetVisualLabCelestialAssetSpriteWidth("Venus"));
    var mercuryRendered = IsVisualLabPlanetAssetRenderable(planetAssets, "mercury", GetVisualLabCelestialAssetSpriteWidth("Mercury"));
    ...
    var spritesDrawn = spec.UseCelestialAssets && assetFilesFound && jupiterRendered && venusRendered && mercuryRendered;
    var fallbackDotsUsed = !spritesDrawn;
    ...
    return new VisualLabOverlayValidation(checks.All(c => c.Passed), width, height, checks, spec.UseCelestialAssets, assetFilesFound, jupiterRendered, venusRendered, mercuryRendered, spritesDrawn, fallbackDotsUsed, celestialAssetsVisible, jupiterSpriteWidth, venusSpriteWidth, mercurySpriteWidth, assetLookup?.Diagnostics);
...
static VisualLabHeroValidation ValidateVisualLabHeroSpec(VisualLabHeroSpec spec, int width, int height, VisualLabPlanetAssetLookup assetLookup)
{
    ...
    var forbiddenEducationalUi = new[] { "date panel", "tips panel", "direction marker", "equipment panel", "observation details", "callout labels", "icons", "informational UI" };
    var noEducationalPanels = forbiddenEducationalUi.All(item => spec.RemovedEducationalElements.Contains(item, StringComparer.OrdinalIgnoreCase));
    ...
    var jupiter = spec.PlanetSprites.FirstOrDefault(p => string.Equals(p.Name, "Jupiter", StringComparison.OrdinalIgnoreCase));
    var venus = spec.PlanetSprites.FirstOrDefault(p => string.Equals(p.Name, "Venus", StringComparison.OrdinalIgnoreCase));
    var mercury = spec.PlanetSprites.FirstOrDefault(p => string.Equals(p.Name, "Mercury", StringComparison.OrdinalIgnoreCase));
    var sizeHierarchy = jupiter is not null && venus is not null && mercury is not null && jupiter.SpriteWidth > venus.SpriteWidth && venus.SpriteWidth > mercury.SpriteWidth;
    var planetArea = spec.PlanetSprites.Sum(p => Math.PI * Math.Pow(p.SpriteWidth / 2.0, 2.0));
    var planetAreaRatio = planetArea / Math.Max(1.0, width * height);
    var titleExact = string.Equals(spec.Title, "PLANET CONJUNCTION", StringComparison.Ordinal) && string.Equals(spec.Subtitle, "JUPITER • VENUS • MERCURY", StringComparison.Ordinal);
    var focalPointStrength = sizeHierarchy && planetAreaRatio >= 0.10 && spec.PlanetSprites.Count == 3;
    var balancedHeroComposition = planetsInsideHeroScene && heroScene.Width >= width * 0.74 && heroScene.Height >= height * 0.58 && spec.PlanetSprites.All(p => p.CenterX >= width * 0.30 && p.CenterX <= width * 0.74);
    ...
    AddCheck("noEducationalUI", noEducationalPanels && spec.DesignMode == "PremiumDocumentaryHeroV2", "Labels, callouts, icons, info panels, tips, direction markers, and educational UI are excluded.");
    AddCheck("titleReadability", titleExact && spec.TitleFontSize >= 78 && spec.SubtitleFontSize >= 36 && RectContains(safe, titleBand), "Title reads as PLANET CONJUNCTION with the Jupiter • Venus • Mercury subtitle inside the safe title band.");
    ...
    AddCheck("focalPointStrength", focalPointStrength, $"Planets are the primary focal point with Jupiter largest, Venus second, Mercury smaller; foreground area ratio is {planetAreaRatio:0.00}.");
    AddCheck("planetAssetsInsideHeroScene", planetsInsideHeroScene, "Large planet assets sit within the central conjunction scene.");
    AddCheck("heroCompositionBalance", balancedHeroComposition, "Planet cluster occupies a balanced central hero field while preserving the Azure background around it.");
    AddCheck("posterQuality", spec.TitleFontSize >= 78 && spec.SubtitleFontSize >= 36 && spec.PlanetSprites.Count == 3 && spec.Composer == "VisualLabHeroComposerV2", "Composition targets premium NASA cover / Netflix science documentary / National Geographic feature-image quality.");
...
static void DrawVisualLabOverlay(IImageProcessingContext ctx, VisualLabOverlaySpec spec, IReadOnlyDictionary<string, VisualLabPlanetAsset> planetAssets)
{
    ...
    {
        DrawVisualLabPlanetMarker(ctx, callout, new PointF(callout.AnchorX, callout.AnchorY), spec.UseCelestialAssets, planetAssets);
    }
...
static void DrawVisualLabHero(IImageProcessingContext ctx, VisualLabHeroSpec spec, IReadOnlyDictionary<string, VisualLabPlanetAsset> planetAssets)
{
    ...
    DrawCenteredTextWithStroke(ctx, title, titleFont, Color.White, Color.ParseHex("#020611").WithAlpha(0.95f), 5, new PointF(titleBand.X + titleBand.Width / 2f, titleBand.Y + 10));
    DrawCenteredTextWithStroke(ctx, spec.Subtitle, subtitleFont, Color.ParseHex("#FFE08A"), Color.ParseHex("#020611").WithAlpha(0.92f), 3, new PointF(titleBand.X + titleBand.Width / 2f, titleBand.Y + spec.TitleFontSize + 26));
...
static async Task<VisualLabPlanetAssetLookup> LoadVisualLabPlanetAssetsAsync(IWebHostEnvironment environment, CancellationToken ct)
{
    var assets = new Dictionary<string, VisualLabPlanetAsset>(StringComparer.OrdinalIgnoreCase);
    var resolvedPaths = new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase);
    var candidatesByObject = new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase);
    var missingAssets = new List<string>();

    foreach (var objectName in new[] { "Jupiter", "Venus", "Mercury" })
    {
        var normalizedName = NormalizeVisualLabCelestialObjectName(objectName);
        var candidates = BuildVisualLabCelestialAssetCandidates(environment, normalizedName);
        candidatesByObject[normalizedName] = candidates;
        var resolvedPath = candidates.FirstOrDefault(File.Exists);
        resolvedPaths[normalizedName] = resolvedPath;
        if (resolvedPath is null)
        {
            missingAssets.Add(normalizedName);
            continue;
        }
    ...
    var diagnostics = new VisualLabPlanetAssetDiagnostics(
        ...
        candidatesByObject.SelectMany(x => x.Value).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(),
        candidatesByObject,
        resolvedPaths,
        missingAssets);

    return new VisualLabPlanetAssetLookup(assets, diagnostics);
}

static Rectangle? GetVisualLabVisibleAlphaBounds(Image<Rgba32> image)
{
    ...
    return maxX < minX || maxY < minY ? null : new Rectangle(minX, minY, maxX - minX + 1, maxY - minY + 1);
}

static int CountVisualLabVisibleAlphaPixels(Image<Rgba32> image)
{
    var count = 0;
    image.ProcessPixelRows(accessor =>
    {
...
static bool IsVisualLabPlanetAssetRenderable(IReadOnlyDictionary<string, VisualLabPlanetAsset>? planetAssets, string key, int requiredMinimumWidth)
    => planetAssets?.TryGetValue(key, out var asset) == true
...
static string ToVisualLabPlanetDisplayName(string normalizedName)
    => normalizedName switch
    {
        ...
        "venus" => "Venus",
        "mercury" => "Mercury",
        _ => CultureInfo.InvariantCulture.TextInfo.ToTitleCase(normalizedName.Replace('-', ' '))
    };

static IReadOnlyList<string> BuildVisualLabCelestialAssetCandidates(IWebHostEnvironment environment, string normalizedName)
    => new[]
    {
...
static VisualLabOverlayRenderDebug BuildVisualLabOverlayRenderDebug(VisualLabOverlaySpec spec, VisualLabPlanetAssetLookup assetLookup, VisualLabOverlayValidation validation)
{
    var objects = spec.PlanetCallouts
        .Select(callout => BuildVisualLabOverlayRenderDebugObject(callout, assetLookup.Assets))
        .OrderBy(x => x.Name, StringComparer.OrdinalIgnoreCase)
        .ToArray();

    return new VisualLabOverlayRenderDebug(
        objects,
        new VisualLabOverlayRenderDebugValidation(validation.CelestialAssetsVisible, validation.SpritesDrawn, validation.FallbackDotsUsed));
}
...
static string NormalizeVisualLabCelestialObjectName(string value)
    => Regex.Replace(value.Trim().ToLowerInvariant(), " +", "-");

static void DrawVisualLabPlanetMarker(IImageProcessingContext ctx, VisualLabOverlayCallout callout, PointF anchor, bool useCelestialAssets, IReadOnlyDictionary<string, VisualLabPlanetAsset> planetAssets)
{
...
{
    if (string.Equals(objectName, "Jupiter", StringComparison.OrdinalIgnoreCase)) return 110;
    if (string.Equals(objectName, "Venus", StringComparison.OrdinalIgnoreCase)) return 95;
    if (string.Equals(objectName, "Mercury", StringComparison.OrdinalIgnoreCase)) return 70;
    return 48;
}
...
public sealed record VisualLabPlanetAssetLookup(IReadOnlyDictionary<string, VisualLabPlanetAsset> Assets, VisualLabPlanetAssetDiagnostics Diagnostics)
{
    public static VisualLabPlanetAssetLookup Empty(IWebHostEnvironment environment)
        => new(new Dictionary<string, VisualLabPlanetAsset>(StringComparer.OrdinalIgnoreCase), new VisualLabPlanetAssetDiagnostics(AppContext.BaseDirectory, environment.ContentRootPath, Array.Empty<string>(), new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase), new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase), Array.Empty<string>()));
}
public sealed record VisualLabPlanetAssetDiagnostics(
    [property: System.Text.Json.Serialization.JsonPropertyName("AppContext.BaseDirectory")] string AppContextBaseDirectory,
    [property: System.Text.Json.Serialization.JsonPropertyName("ContentRootPath")] string ContentRootPath,
    IReadOnlyList<string> CandidatePathsChecked,
    IReadOnlyDictionary<string, IReadOnlyList<string>> CandidatePathsByObject,
    IReadOnlyDictionary<string, string?> ResolvedPathByObject,
...
record AstronomyV31NarrationPreviewRequest(
    string? PlanId,
    string EventType,
    string EventName,
    string ShortTitle,
    ...
    IReadOnlyList<string> ReturnScenes);

record AstronomyV31NarrationPreviewScene(string SceneId, string ScenePurpose, string Narration, AstronomyV31NarrationSceneValidation Validation);
...
{
    public static IReadOnlyList<string> ValidateRequest(AstronomyV31NarrationPreviewRequest request)
{
    var errors = new List<string>();
    if (string.IsNullOrWhiteSpace(request.EventType)) errors.Add("eventType is required.");
    if (string.IsNullOrWhiteSpace(request.EventName)) errors.Add("eventName is required.");
    if (string.IsNullOrWhiteSpace(request.ShortTitle)) errors.Add("shortTitle is required.");
    if (string.IsNullOrWhiteSpace(request.Language)) errors.Add("language is required.");
    if (string.IsNullOrWhiteSpace(request.RegionId)) errors.Add("regionId is required.");
    if (string.IsNullOrWhiteSpace(request.Format)) errors.Add("format is required.");
    if (request.EventMetadata is null || request.EventMetadata.Count == 0) errors.Add("eventMetadata is required.");
    if (request.ReturnScenes is null || request.ReturnScenes.Count == 0) errors.Add("returnScenes is required and must include at least one scene purpose.");
    return errors;
}

public static IReadOnlyList<AstronomyV31NarrationPreviewScene> BuildScenes(AstronomyV31NarrationPreviewRequest request)
{
    var scenes = request.ReturnScenes.Select((purpose, index) =>
    {
        var scenePurpose = string.IsNullOrWhiteSpace(purpose) ? $"scene-{index + 1}" : purpose.Trim();
        var sceneId = $"scene-{index + 1:000}-{SanitizeAstronomyV31Id(scenePurpose)}";
        var narration = BuildAstronomyV31NarrationLine(request, scenePurpose);
        var validation = ValidateAstronomyV31NarrationScene(request, sceneId, scenePurpose, narration);
        return new AstronomyV31NarrationPreviewScene(sceneId, scenePurpose, narration, validation);
    }).ToList();
    return scenes;
}
...
static string BuildAstronomyV31NarrationLine(AstronomyV31NarrationPreviewRequest request, string scenePurpose)
{
    ...
    var p = scenePurpose.ToLowerInvariant();
...
static AstronomyV31NarrationSceneValidation ValidateAstronomyV31NarrationScene(AstronomyV31NarrationPreviewRequest request, string sceneId, string scenePurpose, string narration)
{
...
static bool IsGenericAstronomyV31Fact(AstronomyV31NarrationPreviewRequest request, string narration)
    => !narration.Contains("strawberry", StringComparison.OrdinalIgnoreCase)
       && !narration.Contains("wolf", StringComparison.OrdinalIgnoreCase)
       && !narration.Contains("Gemin", StringComparison.OrdinalIgnoreCase)
       && !narration.Contains("Perseid", StringComparison.OrdinalIgnoreCase)
       && !narration.Contains("Swift-Tuttle", StringComparison.OrdinalIgnoreCase)
       && !narration.Contains("Phaethon", StringComparison.OrdinalIgnoreCase)
...
static bool ContainsLiteralHindiTranslationSmell(string narration)
    => narration.Contains("सबसे अच्छा देखने की खिड़की", StringComparison.OrdinalIgnoreCase)
       || narration.Contains("पेशेवर बंद", StringComparison.OrdinalIgnoreCase)
       || narration.Contains("दिलचस्प तथ्य", StringComparison.OrdinalIgnoreCase);

static string? MetadataText(JsonObject metadata, params string[] keys)
{
    foreach (var key in keys)
    {
        if (metadata.TryGetPropertyValue(key, out var node) && node is not null)
        {
            if (node is JsonArray array) return string.Join(", ", array.Select(n => n?.ToString()).Where(v => !string.IsNullOrWhiteSpace(v))!);
...
static bool IsHindi(string language)
...
static string NormalizeAstronomyV31Sentence(string sentence)
    => Regex.Replace(sentence.Trim().Trim('.', '!', '?', '।'), "\\s+", " ");
}
