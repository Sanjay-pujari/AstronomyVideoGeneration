    ...
    ILogger<Rc2PublishingController> logger) : ControllerBase
{
    ...
    [HttpPost("video")]
    public async Task<IActionResult> Video([FromBody] Rc2PublishVideoRequest request, CancellationToken ct)
        => await Invoke(() => execution.PublishVideoAsync(request, ct));

    [HttpPost("media")]
    public async Task<IActionResult> Media([FromBody] Rc2PublishMediaRequest request, CancellationToken ct)
        => await Invoke(() => execution.PublishMediaAsync(request, ct));
    ...
    private async Task<IActionResult> Invoke<T>(Func<Task<T>> action)
    {
        ...
        catch (Rc2PublishingControlException ex)
        {
            var status = ex.Code switch
            {
                ...
                "RC2_PUBLISH_APPROVAL_REQUIRED" or "RC2_PUBLISH_ALREADY_IN_PROGRESS" => StatusCodes.Status409Conflict,
