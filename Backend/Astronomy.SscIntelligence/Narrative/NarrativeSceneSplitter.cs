...
{
    public NarrativeSceneSplitResult Split(string sceneCode, string sceneTitle, string language, string region, DateTime selectedObservationUtc, DateTime selectedObservationLocal, string? narrationSegmentReference, IReadOnlyList<SkyObjectPosition> resolvedSkyObjects, SpatialCompositionResult spatialComposition, NightWindowResult nightWindow, bool requiresSplit, int maxSplitScenes = 3, int maxTotalScenes = 4)
    {
        var hasMultipleClusters = spatialComposition.Clusters.Count > 1;
        var shouldSplit = spatialComposition.SplitRecommended
            || spatialComposition.CompositionClass == SpatialCompositionClass.ImpossibleGrouping
            || requiresSplit
            || (hasMultipleClusters && spatialComposition.MaxAngularSeparationDeg >= 60d);

        if (!shouldSplit)
        {
            return new(false, "composition-not-impossible", [Build(sceneCode, sceneTitle, SceneIntentType.Grouping, NarrativeSceneRole.Original, resolvedSkyObjects, ["original"], language, region, selectedObservationUtc, selectedObservationLocal, sceneCode, narrationSegmentReference)]);
        }

        var scenes = new List<NarrativeSplitScene>();
        var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var cluster in spatialComposition.Clusters.OrderByDescending(c => c.Objects.Count))
        {
            if (scenes.Count >= maxSplitScenes || scenes.Count >= maxTotalScenes) break;
            var key = string.Join('|', cluster.ObjectNames.OrderBy(x => x, StringComparer.OrdinalIgnoreCase));
        ...
        if (scenes.Count == 0)
        {
            return new(false, "no-unique-clusters", [Build(sceneCode, sceneTitle, SceneIntentType.Grouping, NarrativeSceneRole.Original, resolvedSkyObjects, ["original"], language, region, selectedObservationUtc, selectedObservationLocal, sceneCode, narrationSegmentReference)]);
        }

        return new(true, "spatial-split-recommended", scenes.Take(maxTotalScenes).ToList());
    }
    ...
    {
        var planets = c.ObjectNames.Where(IsPlanet).ToList();
        var isDominant = c.ObjectNames.SequenceEqual(dominant.ObjectNames);
