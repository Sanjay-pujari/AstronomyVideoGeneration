...
public sealed class PlatformContentAnalyticsTests
{
    ...
    private static PlatformAnalyticsCollectionContext Context(string platform, string type, string id)
        => new(Guid.NewGuid(), platform, type, id, null, "Title", DateTimeOffset.UtcNow, 30, "#space", "Austin", DateOnly.FromDateTime(DateTime.UtcNow), ContentType.SpaceNews, null, null);
