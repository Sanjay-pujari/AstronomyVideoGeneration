...
internal static class DocumentarySceneCompositionTestFixtures
{
    ...
    public static DocumentarySceneCompositionRequest Request(IReadOnlyList<DocumentaryMediaAssetResult>? visuals=null, DocumentaryMediaAssetStatus narration=DocumentaryMediaAssetStatus.Planned, DocumentaryMediaAssetStatus subtitle=DocumentaryMediaAssetStatus.Planned)
    {
        ...
        var scene=new DocumentaryMediaScene("scene-1",DocumentaryMediaVariantType.LongEnglish,default,"Mars",1,[narrationBlock],[cue],[visualPrompt],new("timing",0,2000,2000,2000,0,0,Correlation),DocumentarySceneTransition.CrossFade,[reference],Correlation);
...
internal sealed class FakeSceneCompositionProviderBinding : IDocumentarySceneCompositionProviderBinding
{
    ...
    { InvocationCount++;CapturedRequest=request;CapturedCancellationToken=token;if(WaitForCancellation)await Task.Delay(Timeout.Infinite,token);if(Exception is not null)throw Exception;return Output??new(null,Failure); }
}
