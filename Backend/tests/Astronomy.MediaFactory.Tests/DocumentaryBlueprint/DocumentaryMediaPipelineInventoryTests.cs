...
public sealed class DocumentaryMediaPipelineInventoryTests
{
 ...
 [Fact] public void Rejection_inventory_is_contiguous_and_stable(){var values=Enum.GetValues<DocumentaryMediaPipelineRejectionReason>();Assert.Equal(28,values.Length);Assert.Equal(Enumerable.Range(0,28),values.Select(value => Convert.ToInt32(value)));Assert.Equal(nameof(DocumentaryMediaPipelineRejectionReason.OutputManifestMismatch),values[^1].ToString());}
 [Fact] public void Checksum_profile_is_SHA256_schema_1(){var x=new DocumentaryMediaChecksumProfile();Assert.Equal("SHA-256",x.Algorithm);Assert.Equal("1.0",x.SchemaVersion);Assert.Throws<ArgumentException>(()=>new DocumentaryMediaChecksumProfile("MD5"));}
}
