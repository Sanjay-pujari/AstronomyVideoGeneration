...
public sealed class DocumentaryBlueprintBuilderMappingTests
{
    ...
    {
        var request=OrionDocumentaryBlueprintBuilderFixture.Create(); var result=new DocumentaryBlueprintBuilder().Build(request);
        ...
        Assert.Equal(request.Scenes.Count,result.Scenes.Count);
        for (var index=0; index<request.Scenes.Count; index++)
        {
            var input=request.Scenes[index]; var scene=result.Scenes[index];
