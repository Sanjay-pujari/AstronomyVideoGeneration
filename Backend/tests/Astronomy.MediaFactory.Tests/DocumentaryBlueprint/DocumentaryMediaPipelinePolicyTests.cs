...
public sealed class DocumentaryMediaPipelinePolicyTests
{
 ...
 [Theory][InlineData(0,1,1)][InlineData(1,0,1)][InlineData(1,1,0)] public void Attempts_must_be_positive(int v,int n,int c)=>Assert.Throws<ArgumentOutOfRangeException>(()=>new DocumentaryMediaPipelinePolicy(DocumentaryMediaPipelineMode.Execute,true,v,n,c));
