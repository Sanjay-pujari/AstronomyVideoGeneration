...
public static class CrossDomainValidationFixture
{
    ...
    // Display names intentionally match. Identity must rely on EntityId.
    ...
    public static void AssertExactIssue(AstronomyKnowledgeValidationIssue i, string code, string path, string rule, AstronomyKnowledgeValidationSeverity severity, AstronomyKnowledgeDomain domain, AstronomyKnowledgePayloadFamily family) { Assert.Equal(code, i.Code); Assert.Equal(path, i.Path); Assert.Equal(rule, i.RuleId); Assert.Equal(severity, i.Severity); Assert.Equal(domain, i.Domain); Assert.Equal(family, i.Family); }
}
