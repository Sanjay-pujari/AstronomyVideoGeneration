...
export type PageKey = 'dashboard' | 'pipeline-runs' | 'regions' | 'events' | 'alerts' | 'analytics' | 'ai-optimization' | 'optimization-insights' | 'content-calendar' | 'settings';
...
function nav(activePage: PageKey) {
  return `<nav class="top-nav" aria-label="AstroPulse pages">${PAGES.map((page) => `<a class="nav-link ${page.key === activePage ? 'nav-link--active' : ''}" href="/admin/${page.key}" data-router-link>${page.label}</a>`).join('')}</nav>`;
}

function hero(activePage: PageKey) {
  const label = PAGES.find((page) => page.key === activePage)?.label ?? 'Dashboard';
  return `<header class="hero"><div><span class="eyebrow">AstroPulse Mission Control</span><h1>${escapeHtml(label)}</h1><p>Production operations dashboard for astronomy video generation, publishing, scheduling, event ranking, and platform analytics.</p><p>Last refreshed: ${escapeHtml(formatDate((globalThis as { __dashboardLastRefreshedAt?: string }).__dashboardLastRefreshedAt))}</p></div><button class="primary-button" id="refresh-dashboard">Refresh telemetry</button></header>`;
}

function shell(activePage: PageKey, body: string, error?: string) {
  const message = error || '';
  return `<main class="app-shell">${nav(activePage)}<div class="state state--warning">Admin dashboard - operational controls</div>${hero(activePage)}${message ? `<div class="state state--error" role="alert"><strong>Analytics service temporarily unavailable.</strong><span>${escapeHtml(message)}</span></div>` : ''}${body}</main>`;
}
...
function aiOptimizationPage(data: DashboardData) {
  return `<section class="dashboard-grid">${card('Hook scores and recommendations', 'From AI optimization hooks endpoint', data.aiOptimizationByRun.length ? `<pre class="json-preview">${escapeHtml(JSON.stringify(data.aiOptimizationByRun, null, 2))}</pre>` : emptyState('No AI hook recommendations yet.'))}${card('Pending approvals', 'Items waiting for apply/reject', data.aiOptimizationPendingApproval.length ? `<pre class="json-preview">${escapeHtml(JSON.stringify(data.aiOptimizationPendingApproval, null, 2))}</pre>` : emptyState('No pending approvals yet.'))}${card('Publishing recommendations', 'Recommended title/hooks, publish time, hashtags', data.publishingRecommendationsByRun.length ? `<pre class="json-preview">${escapeHtml(JSON.stringify(data.publishingRecommendationsByRun, null, 2))}</pre>` : emptyState('No publishing recommendations yet.'))}<section class="card card--full"><div class="row-actions"><button class="secondary-button" data-ai-generate-now>Generate now</button><button class="secondary-button" data-ai-apply-approved>Apply approved</button><button class="secondary-button" data-ai-reject>Reject</button></div></section></section>`;
}

function optimizationInsightsPage(data: DashboardData) {
  return `<section class="dashboard-grid">${card('Optimization insights', 'Combined analytics and recommendation trends', trendPanel(data.analyticsSummaryCards), 'card--full')}${card('Optimization plan', 'Live /api/optimization/plan output', Object.keys(data.optimizationPlan).length ? `<pre class="json-preview">${escapeHtml(JSON.stringify(data.optimizationPlan, null, 2))}</pre>` : emptyState('No optimization plan returned.'))}<section class="card card--full"><div class="input-row"><input id="optimization-location" placeholder="location" value="global"/><input id="optimization-platform" placeholder="platform" value="youtube"/><button class="secondary-button" data-optimization-load>Load plan</button><button class="secondary-button" data-optimization-apply-preview>Apply preview</button></div></section></section>`;
}
