...
import { escapeHtml } from '../admin/dashboard.js';
...
function alertSignupPage(data: DashboardData) {
  const events = [...data.upcomingEvents, ...data.topEvents].filter((event, index, all) => all.findIndex((item) => item.id === event.id) === index);
  const types = [
    ['MeteorShower', 'Meteor showers'],
    ['FullMoon', 'Full moon / supermoon'],
    ['Eclipse', 'Eclipses'],
    ['SpecialEvent', 'Special event videos'],
    ['DailySkyGuide', 'Daily sky guide reminders']
  ];
  return `<header class="public-page-heading"><span class="eyebrow">Sky alerts</span><h1>Get ready for sky moments worth stepping outside for.</h1><p>Choose your region, language, event types, preferred local alert time, and email address. Subscriptions are saved by the AstroPulse alert backend.</p></header><section class="dashboard-grid"><form class="public-card card--full alert-form" id="alert-preferences" data-alert-preferences novalidate><input type="hidden" name="subscriberId" value=""><div><h2>Alert preferences</h2><p class="muted">Email alerts are supported now. Push and WhatsApp remain future channels.</p></div><label>Email<input name="email" type="email" autocomplete="email" placeholder="you@example.com" required></label><label>Region<select name="regionId" required><option value="">Select a region</option>${data.regions.map((region) => `<option value="${escapeHtml(region.id)}">${escapeHtml(regionLabel(region))}</option>`).join('')}</select></label><label>Language<select name="language" required><option value="en">English</option><option value="hi">Hindi</option></select></label><fieldset><legend>Event types</legend>${types.map(([value, label]) => `<label class="check-row"><input type="checkbox" name="eventTypes" value="${escapeHtml(value)}" ${value === 'DailySkyGuide' ? 'checked' : ''}>${escapeHtml(label)}</label>`).join('')}</fieldset><label>Preferred alert time<input name="preferredAlertTimeLocal" type="time" value="18:00" required></label><label>Minimum event score<input name="minimumEventScore" type="number" min="0" max="1" step="0.01" value="0.65" required></label><div class="state state--empty" data-alert-form-state>Submit to create or update an email sky-alert subscription.</div><div class="alert-actions"><button class="primary-button" type="submit">Save alert subscription</button><button class="secondary-button" type="button" data-alert-test>Send test alert</button><button class="secondary-button" type="button" data-alert-unsubscribe>Unsubscribe</button></div></form><section class="public-card card--full"><h2>Upcoming alert cards</h2>${eventGrid(events, [...data.latestVideos, ...data.latestShorts], 'No alert candidates are available yet. Check back after upcoming events sync.')}</section></section>`;
}
