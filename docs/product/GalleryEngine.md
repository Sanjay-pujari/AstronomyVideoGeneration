...
**Business purpose.** Gallery Engine increases product quality and operational scale for astronomy media generation.

**Product purpose.** It owns the product-facing behavior implied by its name within the event-to-publishing pipeline. Current implementation creates supporting gallery/collage assets. Future RC3 redesign emphasizes educational overlays, carousel generation, step-by-step information hierarchy, and public-page reuse.
...
- Ingest upstream event/story/scene context relevant to Gallery Engine.
...
```mermaid
flowchart TD
A[Inputs] --> B[Gallery Engine Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[Gallery Engine]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Gallery Engine artifact with JSON manifest, localized metadata, and downstream references.
...
```mermaid
flowchart TD
    Current[Current gallery/collage support assets] --> RC3[RC3 educational carousel]
    RC3 --> Overlay[Educational overlays]
    RC3 --> Hierarchy[Information hierarchy]
    RC3 --> Slides[Slide-by-slide story]
    RC3 --> Public[Public portal and social reuse]
```

### Current implementation

The gallery layer currently behaves as a supporting visual product: it packages or composes additional still assets around the event and scene outputs so the campaign has more than a single hero or video frame.

### Future RC3 redesign

The redesigned Gallery Engine should become an educational carousel generator. Each slide should have a clear job: hook, what is happening, where to look, when to observe, how difficult it is, why it matters, and what to do next. Overlays should be factual, sparse, and localized. Information hierarchy should favor one primary fact per slide rather than dense infographic clutter.
