...
**Business purpose.** Media Composer increases product quality and operational scale for astronomy media generation.
...
- Ingest upstream event/story/scene context relevant to Media Composer.
...
```mermaid
flowchart TD
A[Inputs] --> B[Media Composer Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[Media Composer]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Media Composer artifact with JSON manifest, localized metadata, and downstream references.
