...
**Business purpose.** Publishing Engine increases product quality and operational scale for astronomy media generation.
...
- Ingest upstream event/story/scene context relevant to Publishing Engine.
...
```mermaid
flowchart TD
A[Inputs] --> B[Publishing Engine Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[Publishing Engine]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Publishing Engine artifact with JSON manifest, localized metadata, and downstream references.
