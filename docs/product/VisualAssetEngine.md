...
**Business purpose.** VisualAsset Engine increases product quality and operational scale for astronomy media generation.
...
- Ingest upstream event/story/scene context relevant to VisualAsset Engine.
...
```mermaid
flowchart TD
A[Inputs] --> B[VisualAsset Engine Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[VisualAsset Engine]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated VisualAsset Engine artifact with JSON manifest, localized metadata, and downstream references.
