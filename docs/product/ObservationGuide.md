...
**Business purpose.** Observation Guide increases product quality and operational scale for astronomy media generation.

**Product purpose.** It owns the product-facing behavior implied by its name within the event-to-publishing pipeline. Covers direction, time, equipment, visibility, difficulty, and regional guidance.
...
- Ingest upstream event/story/scene context relevant to Observation Guide.
...
```mermaid
flowchart TD
A[Inputs] --> B[Observation Guide Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[Observation Guide]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Observation Guide artifact with JSON manifest, localized metadata, and downstream references.
...
## Observation Guidance Contract

Observation guidance turns astronomy data into human instructions: direction labels, local time windows, equipment recommendations, visibility confidence, difficulty, and regional caveats. The guide must avoid unsafe or overconfident claims. A good output tells a viewer whether to step outside, where to face, when to look, whether binoculars help, and why weather/light pollution/horizon constraints may affect success.
