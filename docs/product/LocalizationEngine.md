...
**Business purpose.** Localization Engine increases product quality and operational scale for astronomy media generation.

**Product purpose.** It owns the product-facing behavior implied by its name within the event-to-publishing pipeline.
...
- Ingest upstream event/story/scene context relevant to Localization Engine.
...
```mermaid
flowchart TD
A[Inputs] --> B[Localization Engine Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[Localization Engine]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Localization Engine artifact with JSON manifest, localized metadata, and downstream references.
