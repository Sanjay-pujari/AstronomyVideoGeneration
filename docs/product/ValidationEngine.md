...
**Business purpose.** Validation Engine increases product quality and operational scale for astronomy media generation.
...
- Ingest upstream event/story/scene context relevant to Validation Engine.
...
```mermaid
flowchart TD
A[Inputs] --> B[Validation Engine Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[Validation Engine]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Validation Engine artifact with JSON manifest, localized metadata, and downstream references.
