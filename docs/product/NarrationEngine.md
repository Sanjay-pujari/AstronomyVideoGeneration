...
**Business purpose.** Narration Engine increases product quality and operational scale for astronomy media generation.

**Product purpose.** It owns the product-facing behavior implied by its name within the event-to-publishing pipeline. Supports scene narration, question-driven narration, Narration V2/V3.1 concepts, SSML/TTS packaging, subtitle alignment, and future voice cloning.
...
- Ingest upstream event/story/scene context relevant to Narration Engine.
...
```mermaid
flowchart TD
A[Inputs] --> B[Narration Engine Core]
...
```mermaid
flowchart TD
    A[Event context] --> B[Narration Engine]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Narration Engine artifact with JSON manifest, localized metadata, and downstream references.
...
```mermaid
journey
    title Viewer comprehension journey
    section Hook
      Ask the core question: 5: Viewer
    section Explain
      Identify objects and timing: 4: Narration
      Explain why it matters: 4: Narration
    section Guide
      Tell viewer where and when to look: 5: Narration
    section Action
      Close with safe observation action: 5: Viewer
```

Narration is question-driven: scenes answer what the viewer is likely asking, not simply what the database knows. Narration V2/V3.1 direction separates director intent, polished narration, final narration cleanup, SSML/TTS packaging, and subtitle generation. SSML and TTS are treated as production artifacts that require package validation, timing consistency, and subtitle alignment. Future voice cloning belongs behind explicit consent, brand safety, and locale-quality gates.
