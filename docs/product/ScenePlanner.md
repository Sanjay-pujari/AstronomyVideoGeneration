...
**Business purpose.** Scene Planner increases product quality and operational scale for astronomy media generation.
...
- Ingest upstream event/story/scene context relevant to Scene Planner.
...
```mermaid
flowchart TD
    A[Event context] --> B[Scene Planner]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Scene Planner artifact with JSON manifest, localized metadata, and downstream references.
