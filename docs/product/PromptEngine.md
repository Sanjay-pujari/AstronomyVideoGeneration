...
**Business purpose.** Prompt Engine increases product quality and operational scale for astronomy media generation.
...
- Ingest upstream event/story/scene context relevant to Prompt Engine.
...
```mermaid
flowchart TD
    A[Event context] --> B[Prompt Engine]
    B --> C[Validation]
    C --> D[Artifact]
```
...
A validated Prompt Engine artifact with JSON manifest, localized metadata, and downstream references.
