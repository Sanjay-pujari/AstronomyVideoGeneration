...
No P7.1A or Phase 1–6 contract, validator, transaction, artifact, test, publication evidence, or
production route was changed. This batch does not create packet persistence, a transaction,
...
The existing `SceneKnowledgePacket` is reused. The additions establish a P7.1B contract version,
...
one matching source-scene index row. Required, Optional, Deferred, and HumanReview dispositions are
...
Viewer questions use the source identity when present and a deterministic scene-role/section editorial
fallback otherwise. The fallback makes no object-specific factual assertion and carries a reason and
...
LocationTimeSafety, Duration, VisualEvidence, LongShortIndependence, and Determinism.
...
The expected certified Orion fixture cardinality is 12 Long packets and 4 Short packets. Test execution
must confirm these counts and the safety, determinism, and independence gates before publication work
begins. The current execution environment does not provide the .NET SDK, so test totals remain
unresolved in this record rather than being fabricated.
