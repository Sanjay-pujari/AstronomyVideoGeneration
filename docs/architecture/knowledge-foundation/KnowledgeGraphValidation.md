...
`AstronomyKnowledgeGraphValidationSet` models nodes, statements, relationships, references, roots, repository metadata, and graph policy. Policies cover connectivity, duplicate nodes/statements/knowledge, external references, statement reachability, unused/orphan entities, cycles, provenance, version, and repository consistency. The cycle rule forbids cycles only for relationship kinds treated as hierarchy/dependency by graph policy; other relationship kinds are not repaired or inferred.
