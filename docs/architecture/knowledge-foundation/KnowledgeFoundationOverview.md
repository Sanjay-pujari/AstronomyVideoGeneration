...
Knowledge is modeled separately from prompts and media so scientific/educational facts, observations, provenance, validation, and graph integrity stay deterministic and reusable before later prompt or media workflows consume them. A statement envelope (`IAstronomyKnowledgeStatement`) carries ID, kind, status, subject, version, audit metadata, and typed payload; payload contracts carry domain-specific data. Architecture is immutable by constructor and record semantics, deterministic by explicit ordering, and dependency-directed.

```mermaid
graph TD
Core[Core contracts] --> Typed[Typed domains and registry]
Core --> Validation[Validation foundation]
Typed --> Validation
Core --> Catalog[Catalog]
Typed --> Catalog
Catalog --> Query[Query model]
Query --> Exec[Query execution]
Catalog --> Exec
Validation --> Cross[Cross-domain validation]
...
Metadata boundaries: catalog and capability snapshots describe contracts and services, not runtime statement storage. Execution boundaries: query engines execute against immutable catalog metadata or caller-supplied in-memory statements only. Validation ownership is divided among constructors, domain validation, cross-domain validation, graph validation, and query validation.
