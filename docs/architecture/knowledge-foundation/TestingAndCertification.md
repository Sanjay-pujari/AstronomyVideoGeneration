...
Task 2.6 certification is documentation- and test-driven. Required focused certification tests live under `Backend/tests/Astronomy.MediaFactory.Tests/KnowledgeFoundation/Certification`. Required command results are recorded in `Task2CompletionReport.md`. The runtime compatibility mechanism remains `IAstronomyKnowledgeFoundationCompatibilityVerifier`; no hosted certification service is added.
