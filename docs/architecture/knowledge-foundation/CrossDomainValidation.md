...
`AstronomyCrossDomainValidationSet` contains relationship-scoped items. Rules inspect related pairs/relationships and ignore unrelated payloads. Execution is deterministic by registered descriptor order. Supported dimensions are entity, classification, epoch, reference context, measurement, orbital/positional, observation/visibility, event participant, and event/temporal consistency. It does not infer science, repair data, or call external services.
