...
`AstronomyKnowledgeValidationIssue` records code, severity, message, path, rule ID, domain, and family. `AstronomyKnowledgeValidationSeverity` separates informational, warning, and error outcomes. `AstronomyKnowledgeValidationMode` supports standard and strict mode behavior. `AstronomyKnowledgeValidationContext` carries mode and minimum severity. Rule descriptors are explicit and sorted by order, rule ID, and type. Validators do not mutate inputs and do not read current time. Constructor validation owns malformed construction; domain validators own a single payload; cross-domain validators own relationship-scoped consistency; graph validators own graph-level integrity; query validators own contradictions.

## Validation ownership matrix
| Defect | Owner |
|---|---|
| malformed strong ID | Constructor |
| invalid enum | Constructor |
| physical-property constraint | Domain validation |
| orbital/positional mismatch | Cross-domain validation |
| event/temporal mismatch | Cross-domain validation |
| broken graph reference | Graph validation |
| duplicate graph identity | Graph validation |
| orphan graph node | Graph validation |
| forbidden graph cycle | Graph validation |
| repository root mismatch | Graph validation |
| query contradiction | Query validator |
