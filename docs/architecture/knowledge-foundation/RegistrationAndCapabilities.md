...
Root registration is `services.AddAstronomyKnowledgeFoundation()`. Sequence: typed knowledge, validation foundation, cross-domain validation, graph validation, catalog, query model, optional query execution, capability snapshot, `IAstronomyKnowledgeFoundationCapabilities`, and optional `IAstronomyKnowledgeFoundationCompatibilityVerifier`. Registrations use singleton lifetimes and `TryAdd`/deduplicated descriptor patterns for idempotency. Individual registrations exist for advanced tests/composition, but root registration is recommended.
