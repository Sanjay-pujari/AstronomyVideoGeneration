...
Contracts: `AstronomyKnowledgeCatalogQuery`, `AstronomyKnowledgeStatementQuery`, `AstronomyKnowledgeQueryPage` (`DefaultLimit` 100, `MaximumLimit` 1000), ordering records, match mode, domain/family/type/subject/kind/status/version/provenance filters, `IAstronomyKnowledgeQueryValidator`, and `AstronomyKnowledgeQueryFingerprint`. Any means at least one value may match; All is single-object conjunction and is rejected when impossible, such as multiple statement kinds. Values are normalized by constructors with distinct deterministic ordering. No fuzzy, full-text, database, or search-provider behavior exists. Default catalog ordering is Kind, Order, Code, Id ascending. Default statement ordering is Id then Revision ascending.
