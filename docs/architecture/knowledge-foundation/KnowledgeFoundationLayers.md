...
| Layer | Responsibility | Inputs | Outputs | Allowed dependencies | Forbidden responsibilities | Registration method | Primary tests |
|---|---|---|---|---|---|---|---|
| Core statement contracts | Strong IDs, versions, status, kind, audit, subject, statement envelope | Constructor values | Immutable values/statements | AstronomyDomain, serialization converters | Validation, catalog, query, integration | N/A | KnowledgeStatementValidationTests |
| Typed payload registry | Explicit built-in descriptors | AstronomyTypedPayloadDescriptor | IAstronomyTypedPayloadRegistry | Core, typed domains | Reflection scanning | AddAstronomyTypedKnowledge | TypedKnowledgeRegistryTests |
| Validation foundation | Issue, severity, context, rule registry, typed validator | Payload/context | ValidationResult | Core, typed domains | Cross-graph repair | AddAstronomyKnowledgeValidation | AstronomyKnowledgeValidation*Tests |
| Domain validation | Payload-owned constraints | Payloads | Issues | Validation foundation | Cross-domain comparison | AddAstronomy*Validation | Domain validation tests |
| Cross-domain validation | Relationship-scoped consistency | AstronomyCrossDomainValidationSet | Issues | Validation foundation, typed domains | Compare unrelated payloads | AddAstronomyCrossDomainValidation | CrossDomain tests |
| Graph validation | Graph integrity policies | AstronomyKnowledgeGraphValidationSet | Issues | Validation, typed registry | Repository persistence | AddAstronomyKnowledgeGraphValidation | Graph tests |
| Catalog | Immutable metadata snapshot | Descriptors/rules/capabilities | Catalog entries | Core, typed registry, validation descriptors | Runtime statement storage | AddAstronomyKnowledgeCatalog | KnowledgeCatalog tests |
| Query model | Typed filters/order/page/fingerprint | Query constructor values | Query objects/validation issues | Catalog | Execution/persistence | AddAstronomyKnowledgeQueryModel | Query tests |
| Query execution | Deterministic in-memory execution | Catalog or caller statements | Results/metadata | Query model, catalog | Repository/database | AddAstronomyKnowledgeQueryExecution | Query Execution tests |
| Integration | Root registration, capabilities, compatibility | IServiceCollection | Registered services/snapshots | Frozen layers | Hosted service/startup blocker | AddAstronomyKnowledgeFoundation | Integration tests |
