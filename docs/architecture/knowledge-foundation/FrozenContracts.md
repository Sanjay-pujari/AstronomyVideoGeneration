...
Frozen through Task 2.5D: core (`KnowledgeId`, `KnowledgeVersion`, statement kinds/status, subject/audit/statement envelope), typed domains (`ITypedAstronomyKnowledgePayload`, domains, families, type IDs, descriptors, registry), validation (`AstronomyKnowledgeValidationIssue`, severity/mode/context/rules/registry/validator), cross-domain (`AstronomyCrossDomainValidationSet`, rules/registry/validator), graph (`AstronomyKnowledgeGraphValidationSet`, nodes/statements/relationships/policies/rules/validator), catalog (entry IDs/kinds/snapshot/builder/catalog), query model (filters/page/order/fingerprint/validator), query execution (catalog/statement engines/results/metadata/status), and integration (root registration, capabilities, compatibility verifier). Additive descriptors/rules/enum-compatible extensions require tests and documentation. Prohibited breaking changes include renamed IDs, changed normalization/equality semantics, removed registrations, changed paging limits/default order, and changed lifetimes without migration. Future breaking changes require explicit migration documentation and compatibility tests.
