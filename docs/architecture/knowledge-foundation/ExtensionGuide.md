...
Add a typed payload by creating an immutable payload implementing `ITypedAstronomyKnowledgePayload`, selecting an existing or justified `AstronomyKnowledgeDomain`/`AstronomyKnowledgePayloadFamily`, assigning a stable `AstronomyKnowledgeTypeId` discriminator, explicitly registering an `AstronomyTypedPayloadDescriptor`, adding serialization tests, adding validation where ownership requires, verifying catalog inclusion, and adding query compatibility tests.

Add a domain rule by defining exact ownership, stable RuleId and issue code, deterministic non-mutating implementation, explicit registration, rule and integration tests, and updating this inventory. Cross-domain rules must define relationship scope and avoid duplicate domain validation. Graph rules must define graph policy and never repair or infer.
