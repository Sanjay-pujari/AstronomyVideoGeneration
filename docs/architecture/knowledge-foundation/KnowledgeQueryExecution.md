...
`IAstronomyKnowledgeCatalogQueryEngine` executes against `IAstronomyKnowledgeCatalog.Snapshot`. `IAstronomyKnowledgeStatementQueryEngine` executes against caller-supplied `IEnumerable<IAstronomyKnowledgeStatement>`. Engines validate before execution, return status/issues/metadata, filter then sort then page, use deterministic ID tie-breakers, enumerate statement sources once, and do not mutate inputs. Rejected queries return rejected status with validation issues. There is no statement repository and no persistence layer.
