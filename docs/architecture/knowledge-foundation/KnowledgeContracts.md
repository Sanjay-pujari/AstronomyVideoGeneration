...
Core public contracts include `KnowledgeId`, `KnowledgeVersion`, `KnowledgeStatementKind`, `KnowledgeFoundationStatus`, `AstronomyEntityReference`, `KnowledgeAuditMetadata`, `ITypedAstronomyKnowledgePayload`, `AstronomyKnowledgeDomain`, `AstronomyKnowledgePayloadFamily`, `AstronomyKnowledgeTypeId`, `AstronomyTypedPayloadDescriptor`, and `IAstronomyTypedPayloadRegistry`. Strong IDs trim, reject blank/control/whitespace where applicable, and compare by value. Records and readonly record structs provide immutable/equality semantics. Constructors own malformed ID, undefined enum, invalid UTC, and invalid range defects. Additive extension may introduce new descriptors/rules; breaking constructor or equality changes require migration.
